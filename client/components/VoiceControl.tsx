...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          // Запрашиваем доступ к микрофону с оптимальными н��стройками
            ...
            setTimeout(() => {
              if (recognitionRef.current && isListening) {
                try {
                  ...
                  console.log(
                    "ℹ️ Распознавание уже запуще��о или недоступно:",
                    error,
                  );
  ...
  const speak = (text: string) => {
    ...
    // Это п����дотвращает повт��рно�� открыти�� панели после команды отклю��ен��я
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не удалось воспрои��вести аудио:", error);
    });
  };
  ...
  const speakWelcomeBack = () => {
    ...
    console.log("���� Начина��м воспроизведение привет��твия");
  ...
  const speakFullAccess = () => {
    ...
    if (commandCooldownRef.current) {
      console.log("⚠️ Принудительно сбрасываем cooldown для speakFullAccess");
      commandCooldownRef.current = false;
    }
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    // Останавливаем лю��ое текущее воспроизведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const speakWithElevenLabs = async (text: string) => {
    ...
    try {
      // Использу��м ElevenLabs API ��ля ��инте��а речи с ва��им ���астомным голосом
  ...
  const speakStopModel = () => {
    // Улучшенная защита - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakStopModel заблокирован - играет ауд��о");
      return;
    }
  ...
  const deactivateStarkLab = () => {
    ...
    const resetState = () => {
      ...
      console.log("✅ Возв��ат к обычно�� теме завершен");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("п��едоставь доступ")
    ) {
    ...
    if (
      ...
      // До��олнит�����льна�� проверка, ч��обы избе��������ть повторных срабатываний
    ...
    if (
      ...
      command.includes("остановить вра��ение")
    ) {
    ...
    // Ра��шире��ная нави��ация ��о се��циям стран��ц��
    if (
