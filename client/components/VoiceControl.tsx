...
export default function VoiceControl({
  ...
  useEffect(() => {
    // Пров��ряем поддер��ку Speech Recognition
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          console.log("Продвинут��е настройки микрофона недоступны");
        }
        ...
        try {
          ...
          navigator.mediaDevices
            ...
            .then((stream) => {
              console.log(
                "���� Получен доступ к микрофону с ��лучшенными настройками",
              );
        ...
        recognitionRef.current.onend = () => {
          ...
          if (isListening) {
            ...
            // Очищаем состояние перед пе��езапуском
            setTranscript("");
            lastCommandRef.current = "";
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (
        ...
        console.log("🧹 Автоматическая очист��а застря��ших блокировок");
        commandCooldownRef.current = false;
  ...
  const speak = (text: string) => {
    ...
    // Если есть cooldown, но не играет аудио, то ����ин��дительн�� сбрасываем cooldown
    ...
    audio.onended = () => {
      setIsSpeaking(false);
      // Сбрасываем состоя��ие через короткую задержку после аудио
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      // Создаем и восп��оизводим ауд��о дл�� команды "откл��чи��ь"
      const audio = new Audio(
  ...
  const speakFullAccess = () => {
    ...
    console.log("🔓 Начинаем ��оспроизведение 'полный доступ'");
  ...
  const speakContinue = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error("❌ Не удал��сь восп��оизв��сти первое аудио:", error);
    });
  };
  ...
  const activateStarkLab = () => {
    ...
    // Останавливаем любое текущее вос��роизведен��е
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const deactivateStarkLab = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ deactivateStarkLab заблокирован - и��рае�� аудио");
      return;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("джарвис отключ��сь") ||
    ...
    // Команда "верни модель" - возвра�� к старой модели (работает с Джарвис и без него)
    if (
    ...
    if (
      ...
      command.includes("продолж��м джарвис") ||
    ...
    if (
      ...
      (command.includes("доброе ��тро") && command.length < 20) ||
    ...
    // Команда "Джарвис ��ак дела" с ответом "Все ��истемы функционируют нормально"
    if (
    ...
    if (
      ...
      if (
        ...
        console.log("❌ ������иагн��стика за��локирова��а:", {
          isSpeaking,
    ...
    if (
      ...
      command.includes("на ����авную страниц��") ||
    ...
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          console.log("Распознаван��е уже запущено или недоступно");
        }
      }
    }
  };
