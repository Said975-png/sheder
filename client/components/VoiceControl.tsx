...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          console.log("Продвинутые настройки микрофона н��доступны");
        }
        ...
        // Наст��ойка прямого доступа к микрофону для лучшего качества
        try {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Показываем пр��межуточный результат с пониженным порогом для дальнего распознавания
          if (
            combinedTranscript &&
          ...
          // Пр��ну��ительно сбрасываем заст��явшие б��окировки если система молчит дольше 5 секунд
          const now = Date.now();
  ...
  const speak = (text: string) => {
    ...
    // Если есть cooldown, но не играет аудио, то ��рин��дительн�� сбрасы��аем cooldown
    ...
    audio.onended = () => {
      setIsSpeaking(false);
      // Сбрасываем состояние через короткую задер��ку после аудио
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      // Создаем и воспроизводим ауд��о дл�� ��оманды "откл��чи��ь"
      const audio = new Audio(
  ...
  const speakFullAccess = () => {
    ...
    console.log("🔓 Начинаем воспроизведение 'полный ��оступ'");
  ...
  const changeToNewModel = () => {
    ...
    // Добавляем эффект сканера с неоном и частица��и
  ...
  const processVoiceCommand = (command: string) => {
    ...
    const forceUnlockTimer = setTimeout(() => {
      console.log("�� Принудительное разблок��рование через 8 секунд");
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      setIsSpeaking(false);
      lastCommandRef.current = "";
    ...
    if (
      ...
      command.includes("джа��вис выключись") ||
    ...
    if (
      ...
      command.includes("давай прод��лжим") ||
    ...
    if (
      command.includes("оригина��ьный джарвис") ||
    ...
    if (
      ...
      command.includes("привет жарвис") || // частые ошибки ра��познавания
    ...
    // Команда "Джарвис ��окрути моде��ь" - запуск вращения модели
    if (
