...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          // Запрашиваем доступ к мик��офо��у с оптимальными настройками
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Пр��ну��ительно сбрасыв��ем застрявшие б��окировки если система молчит дольше 5 секунд
          const now = Date.now();
          ...
          if (
            ...
            if (
              ...
              command.length > 1 // Снижен порог с 2 до 1 символа для лучшего рас��оз��ава��ия коротких команд
            ) {
              console.log(
  ...
  const speakFullAccess = () => {
    ...
    // Создаем и воспроизв��дим аудио для команды "Джар��ис полный доступ"
    const audio = new Audio(
      "https://cdn.builder.io/o/assets%2F236158b44f8b45f680ab2467abfc361c%2Fd8b2e931609e45c3ad40a718329bc1c4?alt=media&token=78714408-6862-47cc-a4ac-8f778b958265&apiKey=236158b44f8b45f680ab2467abfc361c",
    );
  ...
  const speakWithAutoFallback = async (text: string) => {
    ...
    try {
      ...
      console.log("ElevenLabs недоступен, используем бр��узерный TTS");
      ...
      if ("speechSynthesis" in window) {
        return new Promise<boolean>((resolve) => {
          ...
          utterance.onerror = () => {
            ...
            console.error("Ошибка браузерног�� TTS");
            resolve(false);
          };
  ...
  const speakStopModel = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ош��бка воспрои��веде��ия аудио для остановки модели");
    };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Прио��итет: голоса, похожие н��� британский/американский му��ской
      ...
      if (jarvisLikeVoice) {
        ...
        utterance.lang = "ru-RU";
        utterance.pitch = 0.6; // Чуть ниже для лучшего ��вучания русског��
  ...
  const changeToNewModel = () => {
    // Улучшенная защита - р��зрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
    ...
    const changeEvent = new CustomEvent("changeModel", {
      detail: {
        newModelUrl: "https://cdn.builder.io/o/assets%2Fd75af4d8f215499ea8d0f6203e423bd8%2F7e7ff2ca5bfe498ab61759615baa851b?alt=media&token=7869bfeb-bbfa-460d-b8f8-4c63d65277f4&apiKey=d75af4d8f215499ea8d0f6203e423bd8"
      }
    });
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ошибка воспроизведения аудио ��мены модели");
  ...
  const speakLoveYou = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error("❌ Не удалось вос��роизвести аудио 'люблю тебя':", error);
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Команда "Джарвис верни меня обратно" - в��зв��ат к обычной теме
    if (
    ...
    // Команда приветствия "Привет Джарвис" - улучшенное распознавание с ��ащитой от повторов
    if (
    ...
    if (
      ...
      // Дополнитель��ая проверка, чтобы избежать повторных срабатываний
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
    ...
    if (
      ...
      // Если ни��его спе��ифичног����� ��е найдено, поп��обуе�� общий ��о��ск
      if (!found) {
