...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          // @ts-ignore - Увеличиваем усилен��е микрофона
          ...
          if (event.error === "network") {
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                ...
                try {
                  recognitionRef.current.start();
                  console.log("✅ Распознавание восст��новлено");
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 Принудительный ��брос 'говорящего' состояния ��ез аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "✅ Cooldown сброшен, микрофон отключе�� ил�� команда отключен��я - не ��ткрываем панель",
        );
      }
    }, delay);
  };
  ...
  const speak = (text: string) => {
    ...
    // Это предотвращает повторно�� открыти�� панели пос��е команды отклю��ен��я
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        console.log(
          "✅ Ко��анда отключения завершена - панель ос��ается закрытой",
        );
      };
  ...
  const speakThankYou = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не удал��сь восп��оизв��сти аудио благо��а��ности:", error);
    });
  };
  ...
  const speakWithElevenLabs = async (text: string) => {
    // Улучшенная защита - разрешаем если нет активного а��дио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const activateStarkLab = () => {
    ...
    console.log(
      "🔬 Активация лаборатории Старка - начинаем п��следовательность",
    );
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
  ...
  const processVoiceCommand = (command: string) => {
    ...
    setTranscript("");
    // НЕ вызыв��ем onListeningChange во время обработки команды
    ...
    if (
      ...
      console.log("👋 Команда приве��ств��я распознана:", command);
      speakWelcomeBack();
      return;
    }
    ...
    // Команда утреннего приветстви��� "Добро�� утр�� Д��арвис"
    if (
    ...
    if (
      ...
      command.includes("ты зд��сь джарвис") ||
    ...
    if (
      ...
      command.includes("сп��стит��ся к пр��имущества��") ||
