...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            (finalTranscript || combinedTranscript.length > 2) && // Снижен порог �� 5 до 2 символо��
            ...
            if (
              ...
              setTimeout(() => {
                if (!isSpeaking && !audioPlayingRef.current) {
                  console.log("🧹 Очистка сост��яния после отклоненной команды");
                  commandCooldownRef.current = false;
                  lastCommandRef.current = "";
                  setTranscript("");
                }
  ...
  useEffect(() => {
    ...
    const activityTracker = setInterval(() => {
      ...
      // Ес��и микрофон активен но никакой активности более 10 секунд
  ...
  const speakThankYou = () => {
    ...
    audio.onerror = () => {
      ...
      console.error("Ошибка во��п����оизведени�� ���удио бл��годар������о���ти");
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          console.log("Voice ID не найден, использ��ем браузерный TTS");
  ...
  const speakAuthenticJarvis = () => {
    ...
    // НЕ остана��ливаем расп����знавание во врем�� воспроизведения аудио
  ...
  const speakHowAreYou = () => {
    // Улу��шенная защита - разрешаем ��сли нет ����т��вного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const activateStarkLab = () => {
    ...
    // ПЕРВОЕ ау��ио для ком��нды "полная ак��ивация"
    const firstAudio = new Audio(
  ...
  const deactivateStarkLab = () => {
    ...
    console.log("🔄 Возвращ��е�� обычну�� тему");
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Это пред��твращает повторное открытие ��анели
    ...
    // Команда "верни модель" - возвр��т к старой модели (работает с Джарвис и без него)
    if (
    ...
    if (
      ...
      console.log("✅ Кома��да 'верно' распознана:", command);
    ...
    if (
      ...
      command.includes("джарв��с хватит") ||
    ...
    // Умный поиск контента по всем�� сайту
