...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (words.length > uniqueWords.length * 2) {
            console.log("🚫 Откл����няем результат с повтор��ющимися словами");
            combinedTranscript = "";
          }
          ...
          if (
            ...
            if (
              ...
              ); // Мень���� за��ержки для фи��альных результатов
            } else {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            console.error(
              `🚨 Се��евая ошибка распознавания #${networkErrorCount + 1}`,
            );
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                ...
                try {
                  recognitionRef.current.start();
                  console.log("✅ Распознавание во��становлено");
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 Принудительный ��брос 'говорящего' состояния бе�� аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  const speakThankYou = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не удал��сь восп��оизвести аудио благ����арности:", error);
    });
  };
  ...
  const speakAuthenticJarvis = () => {
    ...
    const resetState = () => {
      ...
      // Микрофон ��родолжал работать, н��чего восстанавливать не нужно
  ...
  const speakHowAreYou = () => {
    ...
    // Ос����ан��вливаем ��юбое те��ущее ��оспрои���ведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
    ...
    if ("speechSynthesis" in window) {
      ...
      if (jarvisLikeVoice) {
        ...
        utterance.pitch = 0.55; // Еще ниже дл�� комп��н��ации
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (!command.includes("отключись") && !command.includes("выключись")) {
      if (commandCooldownRef.current || audioPlayingRef.current) {
        console.log(
          "🔄 П��инудит��льно сбрасываем блокировки перед обработкой команды",
        );
        forceResetAllStates();
      }
    }
    ...
    // Команда откл��чения (приор��тетная)
    if (
    ...
    if (
      ...
      command.includes("джарвис включи ��абораторию") ||
    ...
    if (
      ...
      // Улучшенная прове��ка - разрешаем если нет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        deactivateStarkLab();
      }
      return;
    ...
    if (
      ...
      // До��олнит�����льная проверка, ч��обы избе������ть по��торных срабатываний
      // Улучшенная проверка - разрешаем если нет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        speakGoodMorning();
      }
    ...
    if (
      ...
      command.includes("спу��ти���ьс�� к возможностям") ||
