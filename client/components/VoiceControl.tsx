...
export default function VoiceControl({
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            if (
              ...
              ); // Мень��е за����ржки дл�� фи��альных резул��татов
            } else {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 Принудительн���� ��брос 'говорящего' состояния без аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        currentAudioRef.current = null;
        setTranscript("");
        // НЕ открываем панель ��братно после ком��н��ы отключения
        console.log(
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Style: Assistant/Narration (помощник/пове��тв��вание)
  ...
  const testAudioUrls = () => {
    ...
    fetch(url2)
      ...
      .catch((error) => console.error("❌ URL2 недоступ��н:", error));
  };
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.onerror = () => {
          resetState();
          console.error(
            "❌ Оши��ка во��прои����едения второго аудио ��иагностики",
          );
        };
  ...
  const activateStarkLab = () => {
    ...
    // Останавливаем любое текущее вос��роизве��ение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("я верну��ся джарвис") ||
    ...
    if (
      ...
      // �������олнительная провер�����, ��тобы избежать повторных срабат��ва���ий
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
    ...
    if (
      ...
      command.includes("полную диагн��стику систем") ||
