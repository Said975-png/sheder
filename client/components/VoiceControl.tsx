...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            // Проверяем, что команда отличается о�� предыдущей и достаточно длинная (понижен порог)
            if (
              command &&
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Некри��и��еские ошибки - игнорируе�� и ��родолж������ем
            ...
            if (event.error === "no-speech") {
              ...
              // Есл�� сл��шком много no-speech о��ибок ��о��ряд, делаем небольшую паузу
              if (noSpeechCount >= 3) {
                console.log(
          ...
          // Другие оши��ки - ����резапускаем через корот���ое время
            ...
            console.warn(
              "⚠���� Неожиданн��я ош��бка ра��позна����ния:",
              event.error,
              "- перезапу��каем",
            );
            setTimeout(() => {
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "✅ Cooldown сброшен, микрофон отключен ил�� команда отключен��я - не ��ткрываем панел��",
        );
      }
    }, delay);
  };
  ...
  const speakAuthenticJarvis = () => {
    ...
    const resetState = () => {
      ...
      // Микрофон продолжал работать, н����чего в��сстанавливать не нужно
  ...
  const speakSystemDiagnostics = () => {
    ...
    // Улучшенная защита - разрешаем если не��� а��тивного аудио
    if (isSpeaking && audioPlayingRef.current) {
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.play().catch((error) => {
          resetState();
          console.error(
            "❌ Не �����алось воспроизвести второе аудио диагностики:",
            error,
          );
        });
      }, 2000);
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      // Улучшенная проверка - ра��решаем ес��и нет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        speakContinue();
      }
      return;
    ...
    if (
      ...
      // До��олнит�����льная пр��верка, ч��обы избе������ть повторных срабатываний
    ...
    if (
      ...
      if (
        ...
        if (found) {
          speak("По��азываю инфор��ацию о ка��естве");
          return;
        }
      }
    ...
    // Принудительно сбрасываем все блокировки для нераспо��нанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
