...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Другие оши��ки - ����резапускаем через ��орот���ое время
          else {
            console.warn(
  ...
  const speakShutdown = () => {
    ...
    // Останавли����ем Recognition сразу
    if (recognitionRef.current) {
  ...
  const speakWelcomeBack = () => {
    ...
    console.log("👋 Начинаем в��спроизведение привет��твия");
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд��лось воспрои��вести аудио приветс��вия:", error);
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      console.log("🔴 К��манда отключени�� распознана:", command);
    ...
    // Команда "Джарвис верни ��еня обратно" - возврат к обычной теме
    if (
    ...
    if (
      ...
      // �������олнительная провер���а, ��тобы избежать повторных ��рабат��ва���ий
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
