...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          navigator.mediaDevices
            ...
            .catch((e) => {
              console.log(
                "Стандартный доступ к микроф��ну, расширенные настройки недоступны",
              );
            });
        } catch (e) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Показываем промежуточный результат с пониженн��м порогом для дальнего распознавания
          if (
            combinedTranscript &&
          ...
          if (event.error === "network") {
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                console.log(
                  "🔄 В��сстанавливаем распознавани�� после сетево�� ошиб������",
                );
                try {
                  recognitionRef.current.start();
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      ...
      if (isListening) {
        console.log(
          "⚠️ Распозн��вание реч�� может работать некоррект��о без интернета",
        );
      }
    };
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      audio.play().catch((error) => {
        console.error(
          "❌ Не уд����лось воспрои��вес��и аудио ����ключения:",
          error,
        );
        shutdownComplete();
      });
  ...
  const speakFullAccess = () => {
    ...
    // Создаем и ��оспроизв��дим аудио для команды "Джарвис полный доступ"
    const audio = new Audio(
      "https://cdn.builder.io/o/assets%2F236158b44f8b45f680ab2467abfc361c%2Fd8b2e931609e45c3ad40a718329bc1c4?alt=media&token=78714408-6862-47cc-a4ac-8f778b958265&apiKey=236158b44f8b45f680ab2467abfc361c",
    );
  ...
  const speakSystemDiagnostics = () => {
    console.log("🔧 Запуск ��иагн����������стики с��сте����...");
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("��жарвис включи лабораторию") ||
    ...
    if (
      ...
      command.includes("верни меня о��ратно") ||
    ...
    if (
      command.includes("джарви�� покрути модель") ||
    ...
    <div className={inNavbar ? "relative" : "fixed bottom-6 right-6 z-50"}>
      ...
      >
        ...
        <Button
          ...
          className={
            inNavbar
              ? "w-8 h-8 p-0 transition-all duration-300 bg-transparent hover:bg-white/10"
