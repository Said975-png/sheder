...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Обрабатываем только ПОСЛЕДНИЙ резуль��ат, чт��бы н�� накапливать старые
          ...
          // Команда отключения имеет абсолютный приоритет и выполняе��ся всегда
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  // Быстрая очи��тка транскрипт�� после запуска ко����анды
                  setTimeout(() => {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Некри��и��еские ошибки - игнорируе�� и ��родолж������ем
            ...
            if (event.error === "no-speech") {
              ...
              // Есл�� сл��шком мн��го no-speech о��ибок ��одряд, делаем небольшую паузу
              if (noSpeechCount >= 3) {
                console.log(
  ...
  // Функ����я для полного сброса Speech Recognition
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "��� Cooldown сброшен, ми��рофон отключен ил�� команда отключен��я - не ��ткрываем панель",
        );
      }
    }, delay);
  };
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    try {
      ...
      resetState();
      console.error("❌ Не удалось запустить голос Дж��рвиса:", error);
    }
  };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // ����сли н�� нашли подходящий а����л��йский, ищем русский м��жской
  ...
  const deactivateStarkLab = () => {
    // Улучшенная защита - разрешаем е��ли нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Добавля��м очистку т��ймера к концу функц��и
    ...
    if (
      ...
      command.includes("верни меня обр��тно")
    ) {
    ...
    if (
      ...
      console.log("👋 Кома��да приветствия распознана:", command);
      speakWelcomeBack();
      return;
    }
    ...
    if (
      ...
      command.includes("джарвис п��ивет") ||
      ...
      command.includes("жарв��с прив��т") ||
    ...
    if (
      ...
      // �������олнительная про��ер���а, ��тобы избежать повторных срабат��ва���ий
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
    ...
    if (
      ...
      command.includes("полну�� диагностику систем") ||
