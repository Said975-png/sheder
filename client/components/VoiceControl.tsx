...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  setTimeout(() => {
                    ...
                    setTranscript("");
                    // НЕ вызываем onListeningChange, чтобы не от��рывать панель после откл��чения
                    lastCommandRef.current = "";
        ...
        recognitionRef.current.onend = () => {
          ...
          // ВСЕГДА перезапус��аем распознавание, ес��и польз��ватель не отключил м��крофон вручную
          if (isListening) {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    const forceUnlockTimer = setTimeout(() => {
      console.log("⏰ Прину��ительное разблокирование через 8 секунд");
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      setIsSpeaking(false);
      lastCommandRef.current = "";
    ...
    if (
      ...
      // Принудительно выполняем ко��ан��у отключени�� независимо от сост��яния
      speakShutdown();
      return;
    }
    ...
    // Команд�� пр��ветствия "Приве�� Джарвис" - улучшенное распознавание с защи��ой от повторов
    if (
    ...
    if (
      ...
      (command.includes("как дела") && command.length < 20) // ��сли с���ышно ����лько "как дела"
    ) {
