...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            // Если слишком много сете��ых ошибок подряд - отключаем
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                ...
                try {
                  ...
                  console.error("❌ Не удалось вос��тановить распознавание:", error);
                }
              }
            }, retryDelay);
          }
  ...
  const speak = (text: string) => {
    ...
    // Очищае�� транскрипт когда начинаем говорить
    setTranscript("");
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        console.log("✅ Ко��анда отключения завершена - панель ост��ется закрытой");
      };
  ...
  const speakWelcomeBack = () => {
    ...
    // Создаем и в��спроизводим ауд����о для команды "Джарвис я вернулс��"
    const audio = new Audio(
  ...
  const speakThankYou = () => {
    // Разрешаем выпо��нение если нет активного ау��ио
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.volume = 0.95; // Четкая, но ��е резкая громкость
  ...
  const activateStarkLab = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ activateStarkLab заблокирован - игра��т аудио");
      return;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (!command.includes("отключись") && !command.includes("выключись")) {
      if (commandCooldownRef.current || audioPlayingRef.current) {
        console.log("🔄 Принудит��льно сбрасываем блокировки перед обработк��й команды");
        forceResetAllStates();
      }
    }
    ...
    if (
      ...
      // Д��по��нительная про���ерка, чтобы избежать п��вторных с��аба��ыв��ний
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
    ...
    if (
      ...
      command.includes("д��арвис ты ��десь") ||
    ...
    // Команды доб�����влен��я планов в корз��н��
    if (
