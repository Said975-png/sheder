...
export default function VoiceControl({
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      // Только сообщаем о состоянии, если микр��фон все еще актив��н �� это не ��оманда отключения
  ...
  const speakShutdown = () => {
    ...
    // Ос��анавли��аем Recognition сразу
    if (recognitionRef.current) {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        currentAudioRef.current = null;
        setTranscript("");
        // НЕ открываем панель ��братно после коман��ы отключени��
        console.log(
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    if (isSpeaking || isJarvisSpeaking()) {
      console.log("🚫 speakWithJarvis заблокирован - уже и��рает ау��ио");
      return;
    }
  ...
  const speakContinue = () => {
    ...
    // Останавливаем любое текущее воспрои��ведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
    ...
    audio.play().catch((error) => {
      resetState();
      console.error("❌ Не удал��сь вос��роизвести первое аудио:", error);
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("полная активация джа��вис") ||
    ...
    if (
      ...
      command.includes("здрав��твуй джарвис") ||
    ...
    if (
      ...
      if (
        ...
        found = searchAndNavigate(["компа��", "�� нас", "about", "кто мы"]);
        if (found) {
