...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // @ts-ignore - эти с��ойства могут не быть в типах, но р��ботают �� брау��ерах
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Фильтруем повторяющиеся фразы и слишком д��инные результаты
  ...
  // Фу��кция для пол��ого ��броса состояния по���ле ����манды
  ...
  const speak = (text: string) => {
    ...
    // ��озда��м и вос���рои����в��дим ваш новый ау��ио-файл
  ...
  const speakShutdown = () => {
    console.log("🔴 Выполняем команду отключения микр��ф��на");
    ...
    setTimeout(() => {
      console.log("🔊 Создаем аудио для о��ключения");
  ...
  const speakGoodMorning = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error(
        "��е удалось во��произвести аудио утреннего ��рив��т��твия:",
        error,
      );
    });
  };
  ...
  const speakWithElevenLabs = async (text: string) => {
    ...
    try {
      // Используем ElevenLabs API ��ля синте��а р��чи с вашим ��астомным голосом
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Настр��йки макс��мально приближенные к ElevenLabs Jarvis (wDsJlOXPqcvIUKdLXjDs)
  ...
  const speakContinue = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ошибка воспроизве��ения первого аудио");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Фильтруем пу��тые или ����ишко�� короткие команды
    ...
    if (
      ...
      command.includes("активац��я лаборатории джарвис") ||
    ...
    // ��оманда "Джарвис как д��ла" с ответом "Все системы функ��ионируют нормал��но"
    if (
