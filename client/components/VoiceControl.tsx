...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (words.length > uniqueWords.length * 2) {
            console.log("🚫 Откл����яем результат с повторяющимися словами");
            combinedTranscript = "";
          }
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  // Быстрая очистка транскрипта после з��пуска команды
                  setTimeout(() => {
  ...
  const speakAuthenticJarvis = () => {
    ...
    const resetState = () => {
      ...
      // Микрофон продолжал ра��отать, н��чего восстанавливать не нужно
  ...
  const speakSystemDiagnostics = () => {
    ...
    firstAudio.onended = () => {
      ...
      // Ч��рез 2 се��унды воспроизводим вто��ое аудио
      setTimeout(() => {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // При��удительно сбрасываем все блокировки перед обработкой новой коман��ы
    ...
    // ��оманда "Джарвис как дела" с ответ��м "Все системы функ��ионируют нормал��но"
    if (
    ...
    if (
      ...
      command.includes("сп����ститься �� пла��ам") ||
    ...
    // При��удительно сбрасываем все блокировки для нераспознанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
