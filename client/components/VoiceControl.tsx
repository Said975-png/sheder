...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // ��ополнительные нас��ройки для луч��его ра��познава��ия длинны�� фраз
        try {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            console.log(
              "���� Принудите���ьно сбрасываем ��астрявшие блокировки",
            );
  ...
  useEffect(() => {
    ...
    }, 2000); // Пров��ряем каж��ые 2 секунды (чаще для лучшей отзывчивости)
  ...
  const speak = (text: string) => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не удалось воспроизвести ау��ио:", error);
    });
  };
  ...
  const speakShutdown = () => {
    ...
    // Останавли��аем Recognition ��разу
    if (recognitionRef.current) {
  ...
  const speakWelcomeBack = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд����ось ��оспрои��в����сти аудио приветствия:", error);
    });
  };
  ...
  const speakStopModel = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ош��бка воспроиз��едения аудио для остановки модели");
    };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.onerror = () => {
        resetState();
        console.error("Оши��ка син��еза речи");
      };
  ...
  const activateStarkLab = () => {
    ...
    firstAudio.onended = () => {
      console.log("✅ ��ерво�� аудио завершено, ак��ивируем ��абораторию");
  ...
  const deactivateStarkLab = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ош��бка воспроизведени�� аудио возврата");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // КРОМЕ команды отк���ючения, котор��я обрабатываетс�� отд��льно
    ...
    // Команда "Дж��рвис верни меня обратно" - возврат к обычной теме
    if (
    ...
    if (
      command.includes("джарвис �� вернулся") ||
    ...
    if (
      ...
      console.log("🔓 Команда 'пол����й доступ' распознана:", command);
      // Улучшенная проверка - разрешаем если нет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        speakFullAccess();
      }
    ...
    // Команда приветствия "Привет Джарвис" - улучшенное распознавание с защитой от по��торов
    if (
      ...
      // Улучшенная проверка + защита от повторов (10 секунд)
      if (
    ...
    if (
      ...
      command.includes("джарвис вращай ��одель") ||
    ...
    if (
      ...
      command.includes("остановит�� вращ��ние")
    ) {
    ...
    if (
      ...
      // ��оп����лнит��льн��я проверка, чт����бы избе��ать пов��орных сра��атываний
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
    ...
    // ��ро������утка страницы
    if (
      command.includes("прок����тит�� вниз") ||
    ...
    // ВАЖНО: Обработк�� нер��спознанных команд
  ...
  const toggleListening = () => {
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        // П��инудительно очищаем все состояния перед запуском
        setTimeout(() => {
          setTranscript("");
          lastCommandRef.current = "";
        }, 100);
