...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // МАКСИМАЛЬНЫЕ настройки для лучшего распознавания на расст��янии
        ...
        if (
          ...
          try {
            ...
            recognitionRef.current.webkitAutoGainControl = true; // Оставляем для автоматического усиления
            recognitionRef.current.webkitHighpassFilter = false; // Отключаем фильтр для лучшег�� захвата н��зких частот
          ...
          navigator.mediaDevices
            .getUserMedia({
              audio: {
                ...
                echoCancellation: false, // ОТК��ЮЧАЕМ для максимальной чувствительности
                ...
                sampleRate: 48000, // Максималь��ое качество записи
            ...
            .then((stream) => {
              ...
              if (audioTracks.length > 0) {
                ...
                const constraints = {
                  ...
                  sampleRate: { ideal: 48000 }, // Максимальная частота дискретизации
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // ��ильтруем пов��оряющиес�� фразы и слишком длинные р��зультаты
          ...
          if (words.length > uniqueWords.length * 2) {
            console.log(
              "🚫 О��кл����ня��м резу��ьтат с ���овторя��щим��ся словами",
            );
            combinedTranscript = "";
          }
          ...
          // Ко��анда откл��чения имеет абсолютный приоритет �� выполняется в��егда
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            if (networkErrorCount >= 3) {
              console.error(
                "��� Слишком много сетевых оши��ок - отключаем ��аспознавание",
              );
      ...
      setTimeout(() => {
        if (isListening) {
          ...
          if (started) {
            ...
            console.log("⚠️ Н�� удалось перезапустить Speech Recognition");
          }
        }
      }, 200);
    }
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // АГРЕССИВНАЯ очис��ка: если микрофон активен но есть любые блокировки более 3 секунд
    ...
    }, 1500); // Проверяем каждые 1.5 секунды для более быстро�� реакци��
  ...
  const speak = (text: string) => {
    ...
    // ��оз��а������м и вос������рои����водим ваш но��ый ау��ио-файл
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      audio.onerror = () => {
        console.error("Оши��ка во�����роиз��едения ����ди���� отключени������");
        shutdownComplete();
      };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          console.log(
            "Проблема с API ключом ElevenLabs, используем браузерн��й TTS",
          );
  ...
  const speakRotateModel = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error(
        "❌ Не удалось воспроизвести аудио для вра��ения мод��ли:",
        error,
      );
    });
  };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // ��оиск наиболее подходящего голоса д��я ими��ации Jarvis
      const voices = speechSynthesis.getVoices();
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.play().catch((error) => {
          resetState();
          console.error(
            "❌ Не ���д��лось воспроизвести второе ауд���� д��агностики:",
            error,
          );
        });
      }, 2000);
  ...
  const speakContinue = () => {
    ...
    // Пер��ое ау�����о для к��манды "давай продо��жим"
    const audio = new Audio(
  ...
  const changeToOldModel = () => {
    // Улучшенная за��ита - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakLoveYou = () => {
    ...
    console.log("💕 ��оспроизводим ответ на 'люб��ю тебя'");
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.onerror = () => {
          ...
          console.error("❌ Оши��ка воспр��изведения вто��ого аудио акт��ваци��");
        };
  ...
  const deactivateStarkLab = () => {
    ...
    // Уда��яем все лаб��раторные элементы
    ...
    audio.play().catch((error) => {
      resetState();
      console.error("❌ Не уд��лось ��о��произвест�� ��у��ио возврата:", error);
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      console.log("🔴 К��манда откл��че��ия ра����ознана:", command);
    ...
    if (
      ...
      command.includes("жарвис верни пр��шлую") ||
    ...
    if (
      ...
      command.includes("полный доступ джа��вис") ||
    ...
    if (
      ...
      console.log("▶️ К��манда 'давай продолжим' р��с����знана:", command);
    ...
    // Команда приветствия "Привет Джарвис" - улучшенное ��аспознавание с защитой от повторов
    if (
    ...
    if (
      ...
      command.includes("как дела ж��рвис") ||
    ...
    if (
      ...
      command.includes("макс в корзин��") ||
    ...
    if (
      ...
      command.includes("в��из страницы")
    ) {
    ...
    setTimeout(() => {
      ...
      setIsSpeaking(false);
      setTranscript("");
      console.log("✅ Состояния сброшены, система готова �� новым командам");
