...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        if (
          ...
          try {
            ...
            console.log("Расшир��нны�� настройки WebKit недоступны");
          }
        }
          ...
          navigator.mediaDevices
            .getUserMedia({
              audio: {
                ...
                // @ts-ignore - Пр����двинутые ��ас��р��йки для лучшего захв��та звука
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            console.log(
              "���� Принуди��е���ь���о сбрасываем застрявши�� блокиро��ки",
            );
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Другие оши��ки - �������ре��апускаем чере�� корот���ое время
          else {
            console.warn(
      ...
      if (isListening && (commandCooldownRef.current || isSpeaking) && !audioPlayingRef.current && !currentAudioRef.current) {
        ...
        setTimeout(() => {
          if (isListening) {
            const started = safeStartRecognition();
            if (started) {
              console.log("✅ Распознавание принудитель��о перезапущено");
            }
          }
        }, 100);
      }
  ...
  const forceResetAllStates = () => {
    ...
    console.log("✅ ВСЕ СОСТОЯНИЯ СБР��ШЕНЫ");
  };
  ...
  const speakGoodMorning = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error(
        "Оши��ка воспроизведения аудио утреннего пр��ветстви��",
      );
    };
  ...
  const speakWithElevenLabs = async (text: string) => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakWithElevenLabs заблокирован - игра��т ау��ио");
      return;
    }
  ...
  const speakAuthenticJarvis = () => {
    ...
    // Ис��ользуем ваш ��ригиналь��ы�� аудио��айл Джарв��са
    const audio = new Audio(
  ...
  const changeToNewModel = () => {
    ...
    audio.onerror = () => {
      ...
      // Отправляем событие смены моде��и даже при оши��ке ауди��
      window.dispatchEvent(changeEvent);
    };
  ...
  const speakLoveYou = () => {
    // Улучшенная з��щита - разрешаем если ��ет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const deactivateStarkLab = () => {
    ...
    // ТРЕТЬЕ аудио для коман��ы "ве������ меня обр��тно"
    const audio = new Audio(
  ...
  const processVoiceCommand = (command: string) => {
    ...
    console.log("🎯 Обрабатываем команду без с��роса Recognition");
    ...
    if (
      command.includes("верни прошлую ��одель") ||
    ...
    if (
      ...
      command.includes("давай продолжим д��арв��с") ||
    ...
    if (
      ...
      // Улучшенная проверка - разрешаем если нет акт��вно��о аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        speakCorrect();
      }
      return;
    ...
    if (
      ...
      console.log(
        "��� Команда приве��стви�� распознана, врем��ни пр��шло:",
    ...
    console.log("🔄 Сбрасываем состояние для следующей к��манды");
