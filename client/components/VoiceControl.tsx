...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            // Проверяем, что ко��анда отличается от предыдущей и достаточно длинная (понижен порог)
            if (
              command &&
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  setTimeout(() => {
                    console.log("����� Б��с����р�����я очи��тка транскрипт��");
                    setTranscript("");
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                ...
                try {
                  ...
                  setNetworkErrorCount(0); // Сбра��ываем счетчик при успехе
                } catch (error) {
                  console.error(
  ...
  const resetSpeechRecognition = () => {
    if (recognitionRef.current) {
      console.log("�� Полный сбро�� Speech Recognition");
      try {
        recognitionRef.current.stop();
      } catch (error) {
  ...
  // Функция для проверки доступнос��и речевого ��ервиса
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "��� Cooldown сброшен, ми��ро��он отключен ил�� команда отключен��я - не ��ткрываем панель",
        );
      }
    }, delay);
  };
  ...
  const speakThankYou = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не ��дал��сь восп��оизвести аудио благо��а��ности:", error);
    });
  };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.rate = 0.75; // ��ед����н����я, размеренна�� р��чь как �� Джарвиса из фильма
      ...
      if (jarvisLikeVoice) {
        ...
        utterance.pitch = 0.55; // Е��е ниже дл�� комп��нсации
  ...
  const speakCorrect = () => {
    // Улучшенная ��ащита - разре��аем если нет акт��вного ��удио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      console.log("🔄 Команда возврата к о��ычной теме р��спознана:", command);
