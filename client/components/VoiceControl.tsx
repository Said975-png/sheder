...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          // @ts-ignore - Увеличиваем усилени�� микрофона
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            console.error(
              `🚨 Сет��вая ошибка ра��познавания #${networkErrorCount + 1}`,
            );
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                ...
                try {
                  recognitionRef.current.start();
                  console.log("✅ Распознавание в��сстановлено");
          ...
          // Другие оши��ки - ����резапускаем через корот���ое врем��
          else {
            console.warn(
  ...
  const speakShutdown = () => {
    ...
    // Остана��ли��аем Recognition сразу
    if (recognitionRef.current) {
  ...
  const speakWelcomeBack = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд��лось воспрои��вести аудио при��етствия:", error);
    });
  };
  ...
  const speakHowAreYou = () => {
    // Улучшенная защита - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakHowAreYou забло��ирован - играет аудио");
      return;
    }
    ...
    if ("speechSynthesis" in window) {
      ...
      if (jarvisLikeVoice) {
        ...
        if (anyVoice) {
          ...
          utterance.lang = "ru-RU"; // Всегда русс����й язык
        }
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.play().catch((error) => {
          ...
          console.error(
            "❌ Не удалось восп��оизвести второе аудио активации:",
            error,
          );
        });
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      // Принудительно выполняе�� ко��анду отключени�� независимо от сост��яния
      speakShutdown();
      return;
    }
    ...
    if (
      ...
      // Нахо��им и нажимаем ��но����у ко��з���ны
