...
export default function VoiceControl({
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                ...
                try {
                  recognitionRef.current.start();
                  console.log("✅ Расп��знавание восстановлено");
          ...
          // Некри��и��еские ошибки - игнорируе�� и ��родолж������ем
            ...
            if (event.error === "no-speech") {
              ...
              console.log(
                `ℹ️ No-speech о��ибка #${noSpeechCount + 1} - п����од��лжаем ��л����ать`,
              );
          ...
          // Другие оши��ки - ����резапускаем чере�� корот���ое время
          else {
            console.warn(
  ...
  const speakShutdown = () => {
    ...
    // Останав��и��аем Recognition сразу
    if (recognitionRef.current) {
  ...
  const speakWelcomeBack = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд��лось воспрои��вести ау��ио приветствия:", error);
    });
  };
  ...
  const speakContinue = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error("❌ Не удал��сь воспроиз��ести первое аудио:", error);
    });
  };
  ...
  const speakCorrect = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ошибка воспрои��ведени�� втор��го аудио");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("джарвис актива��ия лаборатории") ||
    ...
    // Принудительно сбрасываем ��се блокировки для нераспознанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
