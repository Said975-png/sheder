...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // Доп����нительные настройки для Chrome/WebKit - максимальная чувств��тельн��сть
        if (
          ...
          // @ts-ignore - МАКСИМАЛЬНОЕ количество альтернатив
          ...
          try {
            ...
            recognitionRef.current.webkitEchoCancellation = false; // ОТКЛЮЧ��ЕМ для максимальной чувствительности
            ...
            recognitionRef.current.webkitSensitivity = 0.0; // МИНИМАЛЬНЫЙ порог для максимальной чувствительности
          ...
          navigator.mediaDevices
            .getUserMedia({
              audio: {
                ...
                // @ts-ignore - Продвинутые настройки для максимального захвата звука
            ...
            .then((stream) => {
              ...
              if (audioTracks.length > 0) {
                ...
                const constraints = {
                  ...
                  channelCount: { ideal: 2 }, // Стерео для лучшего кач��ства
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Проверяем на п�����торяющиеся слова (п��изнак на��о��ле��и��)
          ...
          if (
            ...
            if (
              ...
              command.length >= 3 && // Минимум 3 символа для выполнения команды
            ...
            setTimeout(() => {
              if (isListening) {
                ...
                if (!started) {
                  ...
                  // Есл�� не удало��ь ��ерез���пу��тить, поп��обуем еще раз через 500мс
                  setTimeout(() => {
                    if (isListening) {
  ...
  useEffect(() => {
    ...
    const activityTracker = setInterval(() => {
      ...
      // Обновляем время п��следней активности при изменениях
  ...
  const speakShutdown = () => {
    ...
    // ПРИНУДИТЕЛЬНО сбрасываем ВСЕ ��ос��оян���я блокировки ��ля команды ��тключени��
    commandCooldownRef.current = false;
    audioPlayingRef.current = false;
    lastCommandRef.current = "";
    ...
    setTimeout(() => {
      ...
      console.log("▶️ ��ытаемся во����роизвести ау��ио отключения");
      audio.play().catch((error) => {
        console.error(
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    try {
      ...
      await jarvisSpeak(text, {
        ...
        onEnd: () => {
          console.log("✅ Джар����с закон��и�� говори����ь");
          resetState();
        },
        onError: (error) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          console.log("Voice ID не найден, используем брауз��рный TTS");
  ...
  const speakAuthenticJarvis = () => {
    ...
    console.log("🔊 Воспр��из��о���им аудио, но ��став��яем микрофон акт��вным");
  ...
  const speakStopModel = () => {
    // Улучшенн��я защита - разре��аем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakHowAreYou = () => {
    // Улучшенная защита - раз��ешаем ��сли нет ����т��вного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakSystemDiagnostics = () => {
    ...
    firstAudio.onended = () => {
      ...
      // Ч�����ез 2 се��унды вос����оизводим второе а��дио
      setTimeout(() => {
  ...
  const speakContinue = () => {
    // Улучшенная з��щита - разрешаем е��ли нет актив��ого аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (!command.includes("отключись") && !command.includes("выключись")) {
      if (commandCooldownRef.current || audioPlayingRef.current) {
        console.log(
          "⚡ Принудительно сбрасывае�� блокировки перед обработкой команды",
        );
        forceResetAllStates();
      }
    }
    ...
    if (isIncompleteCommand) {
      ...
      setTimeout(() => {
        if (lastCommandRef.current === trimmedCommand) {
          console.log("🔄 ��еполная команда не завершилась, сбрасываем сос��ояние");
          commandCooldownRef.current = false;
          audioPlayingRef.current = false;
          lastCommandRef.current = "";
          setTranscript("");
    ...
    // Коман��а "Джар��ис, полна�� акт��вация" - ак��ивация лаборатории Старка
    if (
    ...
    if (
      ...
      command.includes("джарвис верни пре��нюю модель") ||
    ...
    if (
      ...
      command.includes("верни ��бычную тему") ||
    ...
    if (
      ...
      command.includes("я��вис как дела") ||
    ...
    if (
      ...
      if (
        ...
        command.includes("��и") ||
        command.includes("искус��твенный")
      ) {
        found = searchAndNavigate([
          "технолог",
          "webgl",
