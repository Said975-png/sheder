...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Команда откл��чения имеет абсолютный приоритет �� выполняется всегда
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  setNoSpeechCount(0); // Сб��асываем счетчик пр�� ус��ешном ра�������озна��ании
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Некри��и��еские ��шибки - игнорируе�� и ��родолж��������м
          else if (
    ...
    return () => {
      ...
      // Ос��анавливаем любое воспроизводящ��еся ��удио при раз��онтировани��
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (
        ...
        console.log("🧹 Авто��атическая очист��а застрявших блокировок");
        commandCooldownRef.current = false;
  ...
  const speak = (text: string) => {
    ...
    audio.onended = () => {
      setIsSpeaking(false);
      // Сбрасываем состоян��е через короткую задержку после аудио
  ...
  const speakShutdown = () => {
    console.log("���� Выполняем команд�� отключения микр��фона");

    // ПРИНУДИТЕЛЬНО сбрасываем ВСЕ ��остоян��я блокировки ��ля команды ��тключения
    commandCooldownRef.current = false;
    audioPlayingRef.current = false;
    lastCommandRef.current = "";
    console.log("🔴 Принудительно сбро��или все блокиро��ки");
  ...
  const speakAuthenticJarvis = () => {
    ...
    // О�������т��навливаем любое текущее воспроизведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const speakSystemsOperational = async () => {
    // Сначала пробуем ElevenLabs, если не получается - используем браузерный TTS
    try {
      await speakWithElevenLabs("Все системы функционируют нормально");
    } catch (error) {
      console.log("ElevenLabs недоступен, используем браузерный TTS");
      speakHowAreYou(); // Используем уже готовую функцию с браузерным TTS
    }
  };
  ...
  const speakSystemDiagnostics = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ Диаг��ости��а заблокирована - играет аудио");
      return;
    }
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.play().catch((error) => {
          resetState();
          console.error(
            "❌ Не ���далось воспроизвести второе аудио диагности��и:",
            error,
          );
        });
      }, 2000);
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Фильтруем пу��тые или �����ишком коротки�� коман��ы
    ...
    if (
      ...
      command.includes("джарви�� я здесь") ||
    ...
    // Команды для оригинальн��го голоса Джарвиса (из фил��м��)
    if (
    ...
    setTimeout(() => {
      ...
      setIsSpeaking(false);
      setTranscript("");
      console.log("✅ Со��тояния сброшены, систем�� готова к н��вым командам");
