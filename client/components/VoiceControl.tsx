...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Некри��и����ские ��шибки - игнорируе�� и ��родолж��������ем
          else if (
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      console.log("📵 Потер��но интернет соединение");
      if (isListening) {
        console.log(
  ...
  const speak = (text: string) => {
    // Предотвр��щаем повторное воспроизведение только ��сл�� у��е играет ауди��
    if (isSpeaking) {
  ...
  const speakStopModel = () => {
    ...
    console.log("⏹️ Останавливаем вращение модели с ��удио");
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Similarity Boost: 90 (вы��ок��е сходс����во с ор��гинальным го��осом)
  ...
  const activateStarkLab = () => {
    ...
    firstAudio.onended = () => {
      ...
      // Воспроизв��дим ВТОРОЕ аудио после смены ди��айна
      setTimeout(() => {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("джарвис ��оменяй") ||
    ...
    if (
      ...
      command.includes("жарвис верни прошл��ю модель") ||
    ...
    if (
      ...
      command.includes("жа��вис верни меня обра��но") ||
    ...
    // Умный поиск ��онт��нта по всему сайту
    ...
    // Принудительно сбрасыва��м все блокировки для нераспознанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
