...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          // @ts-ignore - Настройки ��ля улучш��ния зах��ат�� тихой речи
  ...
  const checkSpeechServiceAvailability = async () => {
    try {
      ...
      if (!navigator.onLine) {
        console.log("📵 Нет интернет соединен��я");
        return false;
      }
  ...
  const resetCommandState = (
    ...
    console.log(`⏰ П��ан��руем сброс cooldown че��ез ${delay}мс`);
    setTimeout(() => {
  ...
  const speak = (text: string) => {
    ...
    // ��озда����м и вос�����рои����водим ва�� но��ый ау��ио-файл
  ...
  const speakIAmHere = () => {
    // Разре��аем выполнение если нет активного ау��и��
    if (isSpeaking && audioPlayingRef.current) return;
  ...
  const speakWithElevenLabs = async (text: string) => {
    ...
    try {
      // Используем ElevenLabs API дл�� синтеза р��чи с вашим кастомным голосом
  ...
  const speakAuthenticJarvis = () => {
    // Улучшенная за����и��а - разрешаем если нет акт��вного ��удио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Stability: 20 (низ��ая ст����ильно��ть для более естестве��ной речи)
  ...
  const speakSystemDiagnostics = () => {
    ...
    firstAudio.play().catch((error) => {
      resetState();
      console.error(
        "❌ Не удалось в��спроизвести пе��вое ��удио диагностики:",
        error,
      );
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      command.includes("джарвис верни меня об��атно") ||
    ...
    // Ра��шире��ная нав����ация ��о секциям ст��ан��ц��
    if (
