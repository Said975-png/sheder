...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Фильтруем пов��оряющиес�� ф��азы и слишком длинные р��зультаты
          ...
          if (
            ...
            combinedTranscript.length < 100 && // Фильтр для пред��твр��щен���я накопленных транскриптов
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            setTimeout(() => {
              if (isListening) {
                console.log(
                  "🔄 В��сстанавливаем распозна��ани�� после сетевой ошиб������",
                );
                const started = safeStartRecognition();
                if (started) {
        ...
        if (transcript || commandCooldownRef.current || isSpeaking) {
          ...
          if (recognitionRef.current) {
            try {
              ...
              console.log("Ошибка восстановления после зависания:", e);
            }
          }
  ...
  const speakShutdown = () => {
    ...
    audioPlayingRef.current = false;
    lastCommandRef.current = "";
    console.log("🔴 П��инудительно сбро��или все блокировки");
    ...
    setTimeout(() => {
      console.log("🔊 Создаем ��удио для отключения");
  ...
  const speakFullAccess = () => {
    ...
    if (commandCooldownRef.current) {
      console.log("⚠�� Принудительно сбр��сываем cooldown для speakFullAccess");
      commandCooldownRef.current = false;
    }
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    try {
      ...
      await jarvisSpeak(text, {
        ...
        onError: (error) => {
          console.error("❌ ��шибка речи Дж��рвис���:", error);
          resetState();
        },
      });
    } catch (error) {
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // ����ст����йки максимально ��риближенные �� ElevenLabs Jarvis (wDsJlOXPqcvIUKdLXjDs)
      ...
      // Прио��итет: голоса, похожие н��� британски��/америка��ский мужской
  ...
  const speakCorrect = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ош��бка ��оспроизведени��� втор��го аудио");
    };
  ...
  const deactivateStarkLab = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ deactivateStarkLab забл��киро��а�� - и��рает аудио");
      return;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (isIncompleteCommand) {
      ...
      setTimeout(() => {
        if (lastCommandRef.current === trimmedCommand) {
          console.log("🔄 Неполная команда не завершилась, сбрасываем состояние");
          commandCooldownRef.current = false;
          audioPlayingRef.current = false;
          lastCommandRef.current = "";
          setTranscript("");
    ...
    if (
      ...
      // ��лучшенная проверка - разрешае�� если нет акт��вного ау��ио
      if (!isSpeaking || !audioPlayingRef.current) {
        deactivateStarkLab();
      }
      return;
    ...
    if (
      ...
      console.log("▶️ К��манда 'давай продолжим' р��сп��знана:", command);
    ...
    // Ко��а��да "Верно" - воспр��изводит в��оро�� аудио
    if (
    ...
    if (
      ...
      command.includes("джар��ис из же��езног�� человека") ||
    ...
    if (
      ...
      // Разрешаем только если нет ак��ивного аудио И не в cooldown И последняя команда другая
    ...
    if (
      ...
      command.includes("привет жарвис") || // частые ошибки распознав��ния
    ...
    // Команда "Джарвис ��окрут�� модель" - запуск вращения модели
    if (
    ...
    if (
      ...
      command.includes("как ��оживаешь джарвис") ||
    ...
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          console.log("⚠️ Распознавание уже запущено и��и недоступно:", error);
