...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          // @ts-ignore - Увеличивае�� усиление микрофона
        ...
        try {
          ...
          navigator.mediaDevices
            ...
            .then((stream) => {
              ...
              // Освобождаем поток, т��к как SpeechRecognition создаст свой
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            const command = (finalTranscript || combinedTranscript)
              .toLowerCase()
              .trim();
            console.log("🔍 Анализируем ��ом����ду:", `"${command}"`);
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  setNoSpeechCount(0); // Сбрасываем счетчик пр�� ус��ешном ра��позна��ании
  ...
  // Функ��ия для полного сбро��а Speech Recognition
  ...
  const speakWelcomeBack = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд��лось ��оспрои��вести аудио приветствия:", error);
    });
  };
  ...
  const speakContinue = () => {
    ...
    // Останавливаем любое те��ущее воспроизведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
    ...
    audio.play().catch((error) => {
      resetState();
      console.error("❌ Не ��дал��сь воспроизвести первое аудио:", error);
    });
  };
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.onended = () => {
          ...
          console.log("��� Активац��я лаборатории за��ершена");
        };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // КРОМЕ команд�� отключения, которая обрабатывается отд��льно
    ...
    if (command.includes("заказ") || command.includes("оформ��ть заказ")) {
      navigate("/order");
      speak("Переходи�� �� ��ф��рмлен��ю зак����а");
      return;
    }
