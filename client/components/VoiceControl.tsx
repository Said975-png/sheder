...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onend = () => {
          ...
          // ВС��ГДА перезапус����аем распознава��ие, ес��и польз��ватель не отключил микрофон вручную
          if (isListening) {
  ...
  useEffect(() => {
    ...
    const handleOffline = () => {
      ...
      if (isListening) {
        console.log(
          "⚠️ Распознавание реч�� может ��або��ать некоррект��о без инт��р��ета",
        );
      }
    };
  ...
  const speakShutdown = () => {
    ...
    setIsListening(false);
    onListeningChange?.(false, "");
    console.log("�� Сос��о��н��е listening отключено");
  ...
  const speakAuthenticJarvis = () => {
    ...
    console.log("🔊 Воспр����з��о���им аудио, но остав��яем микрофон акт��вным");
  ...
  const speakHowAreYou = () => {
    // Улучшенная з��щита - разрешаем ��сли нет ����т��вного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const changeToNewModel = () => {
    ...
    // Уведо��ляем о смене модел�� через глобальное событие
  ...
  const deactivateStarkLab = () => {
    ...
    console.log("🔄 ��озвращ��ем обычну�� тему");
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Это пред��твращает повто��ное открытие панели
    ...
    if (
      ...
      command.includes("��т��л��чи ла��оратор��ю") ||
    ...
    if (
      ...
      command.includes("жарвис я в��рнулся") ||
    ...
    if (
      ...
      if (!isSpeaking && !audioPlayingRef.current && !commandCooldownRef.current &&
        ...
        console.log("✅ Вы��олняем команду 'доброе утро'");
    ...
    if (
      ...
      command.includes("джарвис крут�� модель") ||
    ...
    if (
      ...
      // ��ополнительная проверка, чт��бы изб��ж��ть повторных срабатываний
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
    ...
    if (
      ...
      command.includes("спуститься к преимуще��твам") ||
    ...
    // ВАЖНО: Обработ��а нераспознанных команд
