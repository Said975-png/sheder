...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        if (
          ...
          try {
            ...
            recognitionRef.current.webkitSpeechInputCompleteTimeoutMS = 2000; // Таймаут завершения
          } catch (e) {
        ...
        try {
          ...
          navigator.mediaDevices.getUserMedia({
            ...
            console.log("🎤 Получен доступ �� микрофону с улучшенными настройками");
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Используем то��ько новый результат
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      lastCommandRef.current = "";
      setTranscript("");
      setIsSpeaking(false); // Принудительно сбрасываем состояни�� говорения
  ...
  const speakIAmHere = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд��лось воспроизв��ст�� ауд��о ����тве��а:", error);
    });
  };
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    try {
      ...
      await jarvisSpeak(text, {
        onStart: () => {
          console.log("✅ Джарвис начал говори��ь");
        },
        onEnd: () => {
  ...
  const speakAuthenticJarvis = () => {
    ...
    console.log("🔊 Вос��р��изво���им аудио, но остав��яем микрофон активным");
  ...
  const speakContinue = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ошибка воспроизведения п��рвого аудио");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    setTranscript("");
    // НЕ вызываем onListeningChange во время обработки кома��ды
    ...
    if (
      ...
      command.includes("по��ная диагностика") ||
    ...
    // Ра��шире��ная нави��ация ��о секциям с��ран��ц��
    if (
  ...
  const toggleListening = () => {
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        // Принудительно очищаем все состояния п��ред запуском
        setTimeout(() => {
          setTranscript("");
          lastCommandRef.current = "";
        }, 100);
