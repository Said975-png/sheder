...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            // Проверяем, что к��манда отличается от предыдущей и достаточно длинная (понижен порог)
            if (
              command &&
              ...
              ); // Мень��е за��ержки дл�� фи��альных резуль��атов
            } else {
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "��� Cooldown сбр��шен, микрофон отключен ил�� команда отключен��я - не ��ткрываем панель",
        );
      }
    }, delay);
  };
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    // Останавливаем любое текущ��е воспроизведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const speakWithElevenLabs = async (text: string) => {
    // Улучшенная защит�� - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakAuthenticJarvis = () => {
    ...
    // Запоминаем состояние п��ослу��ивания ДО остановки
  ...
  const deactivateStarkLab = () => {
    ...
    const resetState = () => {
      ...
      console.log("✅ Возврат к обычной теме заверше��");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // При��удительно сбрасываем все блокировки перед обработко�� новой команды
    ...
    if (
      ...
      command.includes("полная акт��вация джарвис") ||
    ...
    if (
      ...
      // До��олнит�����льная проверка, ч��обы избе������ть повт��рных срабатываний
    ...
    if (
      ...
      command.includes("жарвис ка�� дела") || // час��ые ошибки рас��ознавани��
