...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          // @ts-ignore - Настройки для улучшения захвата тихой речи
        ...
        if (
          ...
          try {
            ...
            recognitionRef.current.webkitSpeechInputCompleteTimeoutMS = 2000; // Таймаут заве��шения
          } catch (e) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Об��абатываем финальные рез��льтаты или достаточно длинные пром��жуто��ные
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Некри��и��еские ошибки - игнорируе�� и ��родолж������ем
            ...
            if (event.error === "no-speech") {
              ...
              console.log(
                `ℹ️ No-speech о��ибка #${noSpeechCount + 1} - п��одолжае�� слу��ать`,
              );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 Принудительный ��брос 'говорящего' с��стояния без аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  // Функ��ия для проверки доступности речевого сервиса
  ...
  // Фу��к��ия для пол��ого ��броса состояния по���ле ��оманды
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    if (isSpeaking || isJarvisSpeaking()) {
      console.log("🚫 speakWithJarvis заблокирован - уже играет аудио");
      return;
    }
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.lang = "en-US"; // ��нглийский для лучшего качества, потом переклю��им ���� русский
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.onerror = () => {
          resetState();
          console.error(
            "❌ Оши��к�� во��произ��едения второго аудио ��иагностики",
          );
        };
  ...
  const speakCorrect = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakCorrect заблокирован - играет аудио");
      return;
    }
  ...
  const activateStarkLab = () => {
    ...
    firstAudio.onerror = () => {
      ...
      console.error("❌ Ошибка воспроизведения первого а��дио активации");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    const forceUnlockTimer = setTimeout(() => {
      ...
      setIsSpeaking(false);
      lastCommandRef.current = "";
    }, 8000); // ��аксимум 8 секунд на любую команду
    ...
    // Добавляем очистку таймера к концу функц��и
    ...
    // Это ��редотвращает повторное откр��тие панели
    ...
    // Команда "Джарвис, полная активация" - активация лабо��атории Старка
    if (
    ...
    if (
      ...
      command.includes("об��чный режим") ||
    ...
    if (
      ...
      if (
        ...
        console.log(
          "❌ Приветствие заблокиро��ано (��ало времени прошло или играет аудио)",
        );
      }
      return;
    }
    ...
    if (
      ...
      command.includes("системна�� диагностика")
    ) {
    ...
    if (
      ...
      command.includes("с��ус����ит��ся вниз")
    ) {
