...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // Уста��авливаем максимальную чувс��вительность
        try {
          ...
          console.log("Продвинутые настройки микрофона недост��пны");
        }
            ...
            .then((stream) => {
              ...
              if (audioTracks.length > 0) {
                ...
                track
                  ...
                  .catch((e) =>
                    console.log(
                      "Не удалось применить дополните��ьные ограничения:",
                      e,
                    ),
                  );
              }
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Об���абат��ваем финальные рез��л��таты или достаточно длинн��е промежуто��ны��
          ...
          if (
            ...
            console.log(
              "���� Принудите���ьно сбрасываем застрявши�� блокиро��ки",
            );
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  // Полная очист��а состояния ��ома��д�� и ��ерезап��ск Recognition
                  setTimeout(() => {
                    console.log(
        ...
        recognitionRef.current.onend = () => {
          ...
          if (isListening) {
            console.log("🔄 Перезапус��аем р��спо��н��вание...");
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Критическ��я ошиб��а разрешений - отключае����
            ...
            console.error("�� Доступ �� микрофону запрещен");
            setIsListening(false);
            onListeningChange?.(false, "");
          }
          // Некри��и��еские ��шибки - игнорир��е���� и ��родолж��������ем
          else if (
            ...
            // Систе������ автомати��ески ��ереза��ус����ится через onend
          }
          // Другие оши��ки - �������ре��апускаем через корот���ое время
          else {
            console.warn(
              "⚠���� Неожид��нн���я ошибка ра��позна����ния:",
              event.error,
              "- перезапу��каем",
            );
            setTimeout(() => {
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      // Только сообщаем о состоянии, если микр��фон все ��ще ак��ивен �� эт�� не ��������манда о��ключения
      if (isListening && !skipPanelReopen) {
        onListeningChange?.(true, "");
        console.log("✅ Cooldown сброшен, микро���он активен");
      } else {
        console.log(
          "��� Cooldown сброшен, ми���рофон отключен ил�� ��ома��да отключ��н��я - не ��ткрываем панел����",
        );
      }
    }, delay);
  };
  ...
  const speakShutdown = () => {
    ...
    setIsListening(false);
    onListeningChange?.(false, "");
    console.log("�� Состоян��е listening отключе��о");
    ...
    // Останавли��аем Recognition сра��у
    if (recognitionRef.current) {
    ...
    }, 100); // Задержка 100мс дл�� полной ос����новки пр���дыдущего аудио
  };
  ...
  const speakWelcomeBack = () => {
    ...
    console.log("���� Начинаем во��произвед��ние привет��т��ия");
  ...
  const speakFullAccess = () => {
    ...
    console.log("🔓 Начинаем воспроиз��едение 'полный доступ'");
  ...
  const speakGoodMorning = () => {
    ...
    // Останавливаем ��юбое ����екущее воспроизведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
    ...
    audio.onerror = () => {
      resetState();
      console.error(
        "Ошибка воспроизведения аудио утреннего приветствия",
      );
    };
  ...
  const speakIAmHere = () => {
    // Разрешаем выполнение если нет активного ау��ио
    if (isSpeaking && audioPlayingRef.current) return;
    ...
    audio.play().catch((error) => {
      ...
      console.error(
        "Не удалос�� воспроизв����ст���� ауд���о ������тве����а:",
        error,
      );
    });
  };
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    try {
      ...
      await jarvisSpeak(text, {
        ...
        onEnd: () => {
          console.log("✅ Джарвис закон��ил говори����ь");
          resetState();
        },
        onError: (error) => {
  ...
  const speakWithElevenLabs = async (text: string) => {
    ...
    // Оста��авливаем любое текущее вос��р��изв����ени��
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const speakAuthenticJarvis = () => {
    ...
    // ��а��оминаем со��тояние прослу���ивания ДО остановк��
    ...
    setTranscript("");
    // ��Е вызываем onListeningChange во врем�� восп��оизведения ау��ио
  ...
  const speakRotateModel = () => {
    ...
    // Останавлив���ем любое текущее во��произведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const speakStopModel = () => {
    ...
    // Останавливаем любое текущее воспроизведени��
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const speakHowAreYou = () => {
    ...
    // Ос����анавлив��ем ��юбое те����ущее ��оспрои����вед��ние
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.volume = 0.95; // Четкая, но не р��зкая громкость
  ...
  const speakSystemDiagnostics = () => {
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.play().catch((error) => {
          resetState();
          console.error(
            "❌ Не ���далось воспроизвести второе ауд���� диагностики:",
            error,
          );
        });
      }, 2000);
    ...
    firstAudio.play().catch((error) => {
      resetState();
      console.error(
        "❌ Не удалось в��спроизвести пе��вое аудио диагностик��:",
        error,
      );
    });
  };
  ...
  const speakContinue = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakContinue заблокирован - играе�� аудио");
      return;
    }
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ошибка во��произведе��ия перв��го аудио");
    };
  ...
  const changeToNewModel = () => {
    ...
    // ��станавливаем любое текущее во��произведе��ие
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
    ...
    // Добавляем эффект ск��нера с неоном и частицами
  ...
  const changeToOldModel = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error(
        "❌ Не уда��ось воспроизвести аудио возврата модели:",
        error,
      );
  ...
  const activateStarkLab = () => {
    // Улучшенн��я защита - разрешаем если нет ак��ивного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const deactivateStarkLab = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ deactivateStarkLab забл��кирован - и��рает аудио");
      return;
    }
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ош����ка воспроизведения аудио возврат��");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // КРОМЕ команды отк���ючения, котор��я обрабатывае��ся отд��льно
    ...
    // Простая очистка транскрипт���� в начале обр��ботки
    setTranscript("");
    ...
    // Коман��а "Джар��ис, полна�� акт��вация" - ак��ивация ла��оратории Старка
    if (
      ...
      command.includes("яр��ис а���тивация л��боратории") ||
    ...
    if (
      ...
      console.log("��� Команда во��врата к прошлой модели распознана:", command);
      if (!isSpeaking || !audioPlayingRef.current) {
        changeToOldModel();
      }
      return;
    ...
    if (
      ...
      command.includes("��тключи лаборатор��ю") ||
    ...
    // Команда "Джа��вис давай продолж��м" - воспро��зводит ��ервое аудио
    if (
      ...
      console.log("▶️ К��манда 'давай продолжим' распознана:", command);
      // Улучшенная про��ерка - разрешаем если нет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        speakContinue();
      }
      return;
    ...
    if (
      ...
      command.includes("джарвис ��з же��езног�� человека") ||
      command.includes("голо�� джарвиса") ||
    ...
    if (
      ...
      // Дополнительная п��оверка, чтобы избежать повторных срабатываний
      // Разрешаем только если нет активного а��дио И не в cooldown И последняя команда другая
    ...
    if (
      command.includes("привет джарви��") ||
    ...
    // Команда "Джарвис ��окрути модель" - з��пуск вращения модели
    if (
      ...
      command.includes("жарвис покру��и модель") ||
    ...
    // Команда "����атит" - остановка вращения модели
    if (
    ...
    // Команда "Джарвис ��ак дела" с ответом "Все системы функционируют нормал��но"
    if (
    ...
    if (
      ...
      if (
        ...
        console.log("✅ Ус��ов���� выпо��нены, ����п���ск��ем диагностику");
        speakSystemDiagnostics();
      } else {
        console.log("❌ ������иагн��стика заб��окирова��а:", {
          isSpeaking,
    ...
    // П��ов����я��м, с������ержит л����� команда знач��м��е слова
    ...
    if (
      ...
      // Если ничего специфичного не найдено, пробуем общий поиск
      if (!found) {
    ...
    if (
      ...
      // Находи�� и нажимаем кнопку корзины
    ...
    // ВАЖНО: Обработк�� нера��познанных команд
    // Если дошли до этого места - к��манда не была р��спознана
    ...
    setTimeout(() => {
      ...
      setIsSpeaking(false);
      setTranscript("");
      console.log("✅ Состояния сброшены, система готова к нов����м командам");
  ...
  const toggleListening = () => {
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        // Принуд���тельно очищаем все состояния перед запуско��
        setTimeout(() => {
          setTranscript("");
          lastCommandRef.current = "";
        }, 100);
