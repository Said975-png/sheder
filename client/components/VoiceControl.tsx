...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Обрабатываем только ПОСЛЕДНИЙ результат, чтобы н�� накапливать старые
          ...
          // Показывае�� про��ежуточны�� результат только если с��с��ема свободна и это новый короткий контент
          if (
            combinedTranscript &&
          ...
          // Принудительно сбрасываем застрявшие блокировки если система молчит дольше 5 секунд
          const now = Date.now();
          ...
          if (
            ...
            // Проверяем, что команда отличае��ся от предыдущей и достаточно дли��ная
            if (
              command &&
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Другие оши��ки - ����резапускаем через короткое время
          else {
            console.warn(
  ...
  const resetCommandState = (delay: number = 1000, skipPanelReopen: boolean = false) => {
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "✅ Cooldown сброшен, микрофон отключен или команда отключен��я - не открываем панель",
        );
      }
    }, delay);
  };
  ...
  const speakShutdown = () => {
    ...
    // Добавляем небольшую задержку перед созданием нового аудио
    setTimeout(() => {
  ...
  const speakAuthenticJarvis = () => {
    // Множественная защита от повторно��о воспроизведения
    if (isSpeaking || commandCooldownRef.current || audioPlayingRef.current) {
    ...
    // Используем ваш ��ригинальны�� аудиофайл Джарвиса
    const audio = new Audio(
    ...
    const resetState = () => {
      ...
      // Микрофон продолжал работать, ничего восстанавливать не нужно
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      if (jarvisLikeVoice) {
        ...
        utterance.pitch = 0.55; // Еще ниже дл�� компенсации
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      // Принудительно выполняем команду отключения независимо от сост��яния
      speakShutdown();
      return;
    }
    ...
    if (
      ...
      command.includes("джар��ис ��рису��ствуешь") ||
