...
export default function VoiceControl({
            ...
            setTimeout(() => {
              if (recognitionRef.current && isListening) {
                try {
                  ...
                  console.log("✅ ��аспо����авание пе���езапущено");
                } catch (error) {
                  console.log(
  ...
  // Функция для пров��рки доступнос����и речевого сервиса
  ...
  const speak = (text: string) => {
    ...
    // Это предотвращает повторно�� откры��и�� панели после команды отклю��ен��я
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        console.log(
          "✅ Ко��анда отключения ��авершена - панель ост��ется закрытой",
        );
      };
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    // Останавливаем любое текущее в��спроизведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const speakHowAreYou = () => {
    // Улучшенная защита - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakHowAreYou заблок��рован - играет аудио");
      return;
    }
    ...
    if ("speechSynthesis" in window) {
      ...
      if (jarvisLikeVoice) {
        ...
        if (anyVoice) {
          ...
          utterance.lang = "ru-RU"; // Всегда р��сск��й язык
        }
  ...
  const speakSystemDiagnostics = () => {
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        console.log("��️ Запус��ае�� второе ау��ио");
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (!command.includes("о����лючись") && !command.includes("выключись")) {
    ...
    if (
      ...
      navigate("/");
      speak("Пере��одим н�� главную стр��ницу");
      return;
    }
