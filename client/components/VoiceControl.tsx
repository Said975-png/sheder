...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        if (
          ...
          // @ts-ignore - Нас��ройки для дальн��го р��спознавания
          try {
        ...
        recognitionRef.current.onend = () => {
          ...
          // ВС��ГДА перезапус��аем распознав��ние, ес��и польз��ватель не отключил микрофон вручную
          if (isListening) {
  ...
  const speakWelcomeBack = () => {
    ...
    // Создаем и воспроизводим ау������о для команды "Дж��рвис я вернулс��"
    const audio = new Audio(
  ...
  const speakWithJarvis = async (text: string) => {
    // Предотвращаем повторное воспро��зведение
  ...
  const speakStopModel = () => {
    ...
    // Воспроизводим второе аудио (для команды "хв��тит")
    const audio = new Audio(
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Similarity Boost: 90 (в��сок��е сходс����во с ор��гинальным го��осом)
      ...
      // Fallback ес��и Speech Synthesis недоступен
      const resetState = () => {
        setIsSpeaking(false);
        audioPlayingRef.current = false;
        currentAudioRef.current = null;
  ...
  const activateStarkLab = () => {
    ...
    firstAudio.onended = () => {
      ...
      // Воспроизв��дим ВТОР��Е аудио после смены дизайна
      setTimeout(() => {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Команда откл���чени�� (при��ри��е��ная)
    if (
    ...
    if (
      ...
      if (
        ...
        console.log("✅ Ус��ов���� выполнены, ����п���с����ем диагностику");
