...
export default function VoiceControl({
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  // Полная очистка состояния ��оманды и перезапуск Recognition
                  setTimeout(() => {
  ...
  const speak = (text: string) => {
    ...
    if (isSpeaking || commandCooldownRef.current) {
      console.log("🚫 speak заблокирова��:", { isSpeaking, commandCooldown: commandCooldownRef.current });
      return;
    }
  ...
  const speakAuthenticJarvis = () => {
    ...
    // Немедлен��о очищаем транскрипт к��гда начинаем говорить
    setTranscript("");
    onListeningChange?.(true, "");
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("джа��вис я здесь") ||
