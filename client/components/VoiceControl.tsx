...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            combinedTranscript &&
            combinedTranscript.length > 1 && // Снижен порог с 2 до 1 си��вола
          ...
          // Пр��ну����ительно сбрасываем застрявшие б��о��ировки есл�� система молчит дольше 5 секунд
          const now = Date.now();
          ...
          if (
            ...
            if (
              ...
              command.length > 1 // Снижен порог с 2 до 1 символа для ��у��шего рас��оз��ава��ия ко��отких команд
            ) {
              console.log(
        ...
        recognitionRef.current.onend = () => {
          ...
          // В����ГДА перезапус����аем распознавание, ес��и польз��ватель не отключил микрофон вручную
          if (isListening) {
  ...
  // Безопасная функц��я запуска SpeechRecognition
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      // Допо��нительная защита: если система молч��т более 5 секунд, принуд��тельно с����ра��ыв��ем
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      // Только сообщаем о состоянии, если микр��фон все ��ще а����ивен �� это не ���������манда о��ключения
  ...
  const speakThankYou = () => {
    ...
    // Создаем и восп��о��зводим аудио дл������ благод��������ности
    const audio = new Audio(
  ...
  const speakGoodMorning = () => {
    // Улучшенная защита - разрешаем если нет активно��о ��удио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakWithElevenLabs = async (text: string) => {
    // Улу��шен���ая защита - разрешаем если не�� активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakContinue = () => {
    // Улучшенная з��щ��та - разрешаем если нет актив��ого аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("с��енить модель") ||
    ...
    if (
      ...
      command.includes("ты тут дж��рвис") ||
