...
export default function VoiceControl({
  ...
  const playAudio = useCallback((url: string, onComplete?: () => void) => {
    ...
    // Очищаем предыдущую кома��ду чтобы можно было говорить новую во время воспроизведения
    lastCommandRef.current = "";
  ...
  const speakShutdown = () => {
    ...
    // Очищаем команду при о��ключении
    lastCommandRef.current = "";
  ...
  // Автоматическая очистка зависших состоян��й
  useEffect(() => {
