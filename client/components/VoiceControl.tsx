...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        if (
          ...
          // @ts-ignore - Настройки для дальнего ����спознавания
          try {
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  // Бы��тр��я о��истка транскрипта после запуска ко����анды
                  setTimeout(() => {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Критические ошибки с умным восстано��лени��м
          ...
          // Некри��и��еские ошибки - игнорируе�� и ��родолж������ем
            ...
            // Систе��а автоматически пере��а��ус����ится через onend
          }
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      console.log("🔄 Полный сброс ��сех состояний блокировки вы��олн����");
  ...
  const speakAuthenticJarvis = () => {
    ...
    console.log("���� Нач��наем воспроизведение Jarvis ауд��о");
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Similarity Boost: 90 (высок��е сходс����во с оригинал��ным голосом)
  ...
  const speakContinue = () => {
    // Улучшенная защита - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakContinue забло��ирован - играет аудио");
      return;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    const forceUnlockTimer = setTimeout(() => {
      ...
      setIsSpeaking(false);
      lastCommandRef.current = "";
    }, 8000); // Максимум 8 секунд на любую команд��
