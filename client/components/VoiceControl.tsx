...
export default function VoiceControl({
            ...
            setTimeout(() => {
              if (recognitionRef.current && isListening) {
                try {
                  ...
                  console.log("✅ ��а��по��навание пе���еза��ущено");
                } catch (error) {
                  console.log(
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 Принудительны�� ��брос 'говоряще��о' состояния без аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  // ��ункция для проверки доступнос�����и речевого сервиса
  ...
  const speak = (text: string) => {
    ...
    // Это предотвращает ��овторно�� открыти�� панели после команды отклю��ен��я
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не удалось воспро��звести аудио:", error);
    });
  };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.lang = "en-US"; // ��нг��ийский для л��чшего качества, потом переклю�����им ���� русский
      ...
      // ����сл�� н�� нашли подходящий а����л��йс��ий, ище�� русский м��жской
  ...
  const speakSystemDiagnostics = () => {
    console.log("🔧 Запус�� диагн��������стики систе����...");
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.onended = () => {
          ...
          console.log("��� Активац��я лаборатории заве��шена");
        };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    console.log("🎯 Об��абатываем команд�� б��з сброса Recognition");
    ...
    // Ко��анда утреннего приветствия "Доброе утро Джарвис"
    if (
    ...
    if (
      ...
      // Если ни��его специфичног����� не найдено, поп��о��уе�� общий ��оиск
      if (!found) {
