...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          navigator.mediaDevices
            ...
            .then((stream) => {
              ...
              // Освобож��аем поток, ��ак как SpeechRecognition созд��ст свой
            ...
            if (
              ...
              setTimeout(() => {
                if (!isSpeaking && !audioPlayingRef.current) {
                  console.log("🧹 Очистка состояния посл�� отклоненной команды");
                  commandCooldownRef.current = false;
                  lastCommandRef.current = "";
                  setTranscript("");
                }
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Критическ��я ошиб���� разрешений - отключае����
    ...
    if (recognitionRef.current) {
      ...
      setTimeout(() => {
        if (isListening) {
          const started = safeStartRecognition();
          if (started) {
            console.log("✅ Speech Recognition перезапущен и очи������н");
          } else {
  ...
  const resetCommandState = (
    ...
    console.log(`⏰ П��ан��руем сбро�� cooldown через ${delay}����`);
    setTimeout(() => {
  ...
  const speakFullAccess = () => {
    ...
    audio.onerror = () => {
      ...
      console.error("��шибка в��спроизведения аудио п��лного доступа");
    };
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    try {
      console.log("���️ Говорит Дж��рви��:", text);
  ...
  const speakSystemDiagnostics = () => {
    ...
    // Улучшенная защита - разр��шаем если не���� активного а��дио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const changeToNewModel = () => {
    ...
    audio.play().catch((error) => {
      ...
      // Отправляем событие смены модели даже ��ри ошибке
      window.dispatchEvent(changeEvent);
    });
  };
  ...
  const activateStarkLab = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ activateStarkLab заблокирован - иг��ает ��удио");
      return;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (trimmedCommand.length < 3) {
      ...
      // Принудительно сб����сываем состояние для коротких команд
      setTimeout(() => {
        commandCooldownRef.current = false;
        audioPlayingRef.current = false;
        lastCommandRef.current = "";
    ...
    if (
      ...
      command.includes("в����нуть модель") ||
    ...
    if (
      ...
      // ��луч��енная проверка - разрешае�� если нет акт��вного ау��ио
      if (!isSpeaking || !audioPlayingRef.current) {
        deactivateStarkLab();
      }
      return;
    ...
    // Ко��анда "Как де��а" (общая, без имени �����а��вис)
    if (
  ...
  const toggleListening = () => {
    if (isListening) {
      ...
      onListeningChange?.(false, "");
      console.log("✅ Микрофон откл��чен, все состояния очищены");
    } else {
