...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        if (
          ...
          // @ts-ignore - Нас��ройки для даль��его р��спознавания
          try {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Об���абат��ваем финальн��е рез��л��таты или достаточно длинн��е промежуто��ны��
  ...
  // Фу��кция дл�� п��л���ого ���броса состояния по����л�� ��оманды
  ...
  const speakAuthenticJarvis = () => {
    ...
    const resetState = () => {
      ...
      console.log("✅ Ауди�� завершено, микрофон остается а��тивным");
  ...
  const speakSystemDiagnostics = () => {
    ...
    // Улучшенная защита - разрешаем если не��� акт��вного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakLoveYou = () => {
    ...
    console.log("💕 Воспроизводим ответ на 'любл�� тебя'");
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // ��чищаем тайм��р если команда успешно завершится
    ...
    if (
      ...
      command.includes("джарвис верни старую мо��ель") ||
    ...
    if (
      ...
      // Улучшенная проверка - разрешаем если нет активного ауд��о
      if (!isSpeaking || !audioPlayingRef.current) {
        speakFullAccess();
      }
      return;
    ...
    if (
      ...
      if (!isSpeaking && !audioPlayingRef.current && !commandCooldownRef.current &&
        ...
        lastCommandRef.current = "доброе у��ро";
        speakGoodMorning();
      } else {
    ...
    if (
      ...
      command.includes("д��арвис п��верни модель") ||
    ...
    if (
      ...
      command.includes("ярвис как де��а") ||
