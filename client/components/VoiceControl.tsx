...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            if (
              ...
              ); // Мень��е за��ержки дл�� фи��а��ьных резул��татов
            } else {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                ...
                try {
                  recognitionRef.current.start();
                  console.log("✅ Распознавание восстано��лено");
          ...
          // Другие оши��ки - ����резапускаем через корот���ое в��емя
          else {
            console.warn(
  ...
  const speakWelcomeBack = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд��лось воспрои��вест�� аудио приветствия:", error);
    });
  };
  ...
  const deactivateStarkLab = () => {
    // Улучшенная защита - разрешаем если ��ет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Уста��авливаем таймер на сброс блоки��овок для ЛЮБОЙ команды
    ...
    if (
      ...
      // �������олнительная провер���а, ��тобы избежать повторных сраб��т��ва���ий
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
