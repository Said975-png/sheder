...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        if (
          ...
          // @ts-ignore - Увеличиваем количество ����ьтернати��
        ...
        // Дополнительные нас��ройки для луч��его ра��познава��ия дли��ны�� фраз
        try {
  ...
  const speakIAmHere = () => {
    // Разре��аем выполн��ние если нет активного ауди��
    if (isSpeaking && audioPlayingRef.current) return;
  ...
  // Новая функ��ия для синт��зиров��нного голоса Джарвиса
  ...
  const speakAuthenticJarvis = () => {
    // Улучшенная за����и��а - разре��аем если нет акт��вного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const speakSystemDiagnostics = () => {
    ...
    firstAudio.play().catch((error) => {
      resetState();
      console.error(
        "❌ Не удалось ����спроизвести пе��вое аудио диагностики:",
        error,
      );
    });
  };
  ...
  const speakLoveYou = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ speakLoveYou ��абло��ирован - играет аудио");
      return;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("��жарвис я здесь") ||
    ...
    // Команда "Джарвис давай продолжим" - воспроизвод��т первое аудио
    if (
    ...
    if (
      ...
      console.log("✅ Ком��нда 'верно' распознана:", command);
      // Улучшенная проверка - разрешаем если нет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        speakCorrect();
      }
    ...
    if (
      ...
      command.includes("джарв��с привет") ||
    ...
    if (
      ...
      // Улучшенн��я проверка - разрешаем если нет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        speakLoveYou();
      }
      return;
