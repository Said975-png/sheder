    ...
    setTimeout(() => {
      ...
      audio.onended = () => {
        ...
        setTimeout(() => {
          ...
          console.log("✅ БЛОКИРОВКА СНЯТА + СОСТОЯНИЕ ОЧИЩЕНО: Готов к новым ко��а��дам");
  ...
  const speakHowAreYou = () => {
    if ("speechSynthesis" in window) {
      ...
      utterance.onend = () => {
        ...
        // СНИМАЕМ БЛОКИРОВКУ после TTS + полная очистка
        setTimeout(() => {
      ...
      utterance.onerror = () => {
        ...
        // СНИМАЕМ БЛОКИРОВКУ при ошибке TTS + полная очистка
        lastCommandRef.current = "";
        setTranscript("");
        isPlayingAudioRef.current = false;
