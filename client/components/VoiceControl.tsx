...
export default function VoiceControl({
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            // П��оверяем, что коман��а отличается от предыдущей и достаточно длинная (понижен порог)
            if (
              command &&
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        currentAudioRef.current = null;
        setTranscript("");
        // НЕ открываем панель ��братно после ��оман��ы отключения
        console.log(
  ...
  const speakContinue = () => {
    ...
    // Останавливаем любое текущее во��произведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const activateStarkLab = () => {
    ...
    firstAudio.onended = () => {
      ...
      // До��а��ляем активационный оверлей
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      // �������олнительная провер���а, ��тобы избежать повторны�� срабат��ва���ий
      if (
        !isSpeaking &&
        !commandCooldownRef.current &&
        !audioPlayingRef.current
    ...
    if (
      ...
      if (
        ...
        found = searchAndNavigate(["компан", "�� нас", "about", "��то мы"]);
        if (found) {
