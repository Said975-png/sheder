...
export default function VoiceControl({
  ...
  const playAudio = useCallback((url: string, onComplete?: () => void) => {
    // Защит�� от множественного воспроизведения
    if (isSpeaking) {
    ...
    setTimeout(() => {
      ...
      audio.onended = () => {
        ...
        setTimeout(() => {
          // Полностью очищаем все старые команды и состояния
          lastCommandRef.current = "";
          setTranscript("");

          // Снимаем блокировку
          isPlayingAudioRef.current = false;
          console.log("✅ БЛОКИРОВКА СНЯТА + СОСТОЯНИЕ ОЧИЩЕНО: Готов к новым ко��андам");
      ...
      audio.onerror = () => {
        ...
        // СНИМАЕМ БЛОКИРОВКУ при ошибке + полная очистка
        lastCommandRef.current = "";
        setTranscript("");
        isPlayingAudioRef.current = false;
          ...
          setTimeout(() => {
            ...
            if (recognitionRef.current) {
              try {
                ...
                console.log("ℹ️ Ошибка возо��новления распознавания:", error);
              }
            }
          }, 500);
        }
  ...
  const speakHowAreYou = () => {
    if ("speechSynthesis" in window) {
      ...
      utterance.onend = () => {
        ...
        // СНИМАЕМ БЛОКИРОВКУ после TTS
        setTimeout(() => {
          isPlayingAudioRef.current = false;
          console.log("✅ БЛОКИРОВКА TTS СНЯТА: Результаты распознавания снова обрабатываются");
        }, 1000);
