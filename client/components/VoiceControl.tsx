...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          // @ts-ignore - Увеличиваем усил��ние микрофона
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (words.length > uniqueWords.length * 2) {
            console.log("🚫 Откл����няем результат с повторяющи��ися словами");
            combinedTranscript = "";
          }
          ...
          if (event.error === "network") {
            ...
            setTimeout(() => {
              if (isListening && recognitionRef.current) {
                console.log(
                  "🔄 Восстанавливаем распознавание после сетевой ошибк��",
                );
                try {
                  recognitionRef.current.start();
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 П��инудительны�� ��брос 'говорящего' состояния без аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  const speak = (text: string) => {
    ...
    setTranscript("");
    // НЕ вызываем onListeningChange во вр��мя восп��оизведе��ия аудио
  ...
  const speakShutdown = () => {
    ...
    if (currentAudioRef.current) {
      ...
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
      currentAudioRef.current = null; // ��чищаем сс��лку
    }
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        currentAudioRef.current = null;
        setTranscript("");
        // НЕ открываем панель ��брат��о после коман��ы отключения
        console.log(
  ...
  const speakHowAreYou = () => {
    // Улучшенная ��ащита - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("джарви�� выключи лабораторию") ||
