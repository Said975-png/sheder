...
export default function VoiceControl({
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (
            ...
            if (
              ...
              ); // Мень��е за��ержки дл�� фи��альных результат��в
            } else {
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "��� Cooldown сбро��ен, ми��рофон ��тключен ил�� команда отключен��я - не ��ткрываем панель",
        );
      }
    }, delay);
  };
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        console.log(
          "✅ Ко��анда отключения заверш��на - панель остается закрытой",
        );
      };
  ...
  const activateStarkLab = () => {
    ...
    console.log(
      "🔬 Активац��я лаборатории Старка - начинаем последовательность",
    );
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.play().catch((error) => {
          ...
          console.error(
            "❌ Не удалось воспроизве��ти второе аудио активации:",
            error,
          );
        });
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // Простая очистка транскрипт���� �� начале обработки
    setTranscript("");
    ...
    // Коман��а "Джарвис давай продолжим" - воспроизводит первое аудио
    if (
    ...
    // Команда утреннего приветствия "Д��брое утро Джарвис"
    if (
    ...
    if (
      ...
      command.includes("зареги��трироваться")
    ) {
      navigate("/signup");
