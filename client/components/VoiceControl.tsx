...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onend = () => {
          ...
          // ВС��ГДА перезапус���аем распознавание, ес��и польз��ватель не от��лючил микрофон вручную
          if (isListening) {
  ...
  useEffect(() => {
    ...
    const activityTracker = setInterval(() => {
      ...
      // Если микрофон активен но никакой активности бол��е 10 секунд
  ...
  const speakShutdown = () => {
    ...
    setTimeout(() => {
      ...
      const shutdownComplete = () => {
        ...
        console.log(
          "✅ Ко��анда отключ��ния зав��р����ена - панель ��ст��ется закрыт��й",
        );
      };
  ...
  const speakWelcomeBack = () => {
    ...
    // ��оздаем и в��спроизводим ау��������о для команды "Джарвис я вернулс��"
    const audio = new Audio(
  ...
  const speakAuthenticJarvis = () => {
    ...
    // ��апоминаем со��тоя��ие прослу���ивания ДО остановк��
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.pitch = 0.7; // Сред����-ни��кий тон для ����втор��т�������ос��и
      ...
      if (jarvisLikeVoice) {
        ...
        utterance.rate = 0.7; // Е������ м��д��енне�� ��ля б��льш��й солидно��ти
      }
  ...
  const speakSystemDiagnostics = () => {
    ...
    // ��станавл����ваем любое тек��щее воспроиз��ед��ние
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const activateStarkLab = () => {
    ...
    if (isSpeaking && audioPlayingRef.current) {
      console.log("❌ activateStarkLab заблокирован - иг��ае�� аудио");
      return;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (!command.includes("отключись") && !command.includes("выключись")) {
      if (commandCooldownRef.current || audioPlayingRef.current) {
        console.log(
          "⚡ Принудительно сбрасываем б��окировки перед обработкой команды",
        );
        forceResetAllStates();
      }
    }
    ...
    if (
      ...
      // Улучшенна�� проверка - разрешаем если ��ет активного аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        activateStarkLab();
      }
      return;
    ...
    if (
      ...
      console.log("��� К��ман��а возврата к прошлой модели распознана:", command);
      if (!isSpeaking || !audioPlayingRef.current) {
        changeToOldModel();
      }
      return;
    ...
    const searchAndNavigate = (
      ...
      // Поиск по data-section ат��ибутам
    ...
    if (
      ...
      if (
        ...
        if (found) {
          speak("Показываю информацию �� качестве");
          return;
        }
      }
