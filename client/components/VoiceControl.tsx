...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Другие оши��ки - ����резап��скаем через корот���ое время
          else {
            console.warn(
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 Пр��нудительны�� ��брос 'говорящего' состояния без аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  const speakShutdown = () => {
    ...
    if (currentAudioRef.current) {
      ...
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
      currentAudioRef.current = null; // Очищаем с����лку
    }
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    if (isSpeaking || isJarvisSpeaking()) {
      console.log("🚫 speakWithJarvis забло��ирован - уже и��рает аудио");
      return;
    }
  ...
  const speakAuthenticJarvis = () => {
    ...
    // О�����танавливаем любое текущее в��спроизведение
    if (currentAudioRef.current) {
      currentAudioRef.current.pause();
      currentAudioRef.current.currentTime = 0;
    }
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("отключи лаборат��рию") ||
    ...
    if (
      ...
      command.includes("давай продолжим ��жарвис") ||
    ...
    // Команды для ориги��ального голоса Джарвиса (из фильм��)
    if (
    ...
    if (
      ...
      command.includes("привет жарвис") || // ��астые ошибки распознавания
    ...
    // Принудительно сбр��сываем все блокировки для нераспознанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
