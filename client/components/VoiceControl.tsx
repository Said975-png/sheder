...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // @ts-ignore - эти свойства могут не быть в т��пах, но работают в браузерах
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          if (words.length > uniqueWords.length * 2) {
            console.log("🚫 О��кл����няем результат с ��овторяющимися словами");
            combinedTranscript = "";
          }
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  // Полная очистка состояния ��ома��ды �� ��ерезапуск Recognition
                  setTimeout(() => {
            ...
            setTimeout(() => {
              if (recognitionRef.current && isListening) {
                try {
                  ...
                  // Есл�� не удало��ь перез���пу��тить, попро��уем еще раз через 500мс
                  setTimeout(() => {
                    if (recognitionRef.current && isListening) {
                      try {
                        recognitionRef.current.start();
  ...
  const resetCommandState = (
    ...
    console.log(`⏰ П��анир��ем сброс cooldown через ${delay}мс`);
    setTimeout(() => {
      ...
      // Только сообщаем о состоянии, если микр��фон все еще ак��ивен �� это не ����оманда отключения
  ...
  const speak = (text: string) => {
    ...
    // ��озда��м и вос���рои����водим ваш новый ау��ио-��айл
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    if (isSpeaking || isJarvisSpeaking()) {
      console.log("🚫 speakWithJarvis заблокирован - уже ����рает а��дио");
      return;
    }
    ...
    try {
      ...
      resetState();
      console.error("❌ Не удалось запустить голос Джа��виса:", error);
    }
  };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      // Stability: 20 (низкая ст��бильно��ть дл�� более естестве��ной речи)
  ...
  const processVoiceCommand = (command: string) => {
    ...
    }, 500); // Небольшая задержка чтобы ��збеж��ть конфликтов
  };
