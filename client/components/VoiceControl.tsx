...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // Настройка пря��ого досту��а к микрофону для лучш��го качества
        try {
          ...
          navigator.mediaDevices
            ...
            .then((stream) => {
              console.log(
                "���� Получен до��туп к микрофону с улучшенными ��астройками",
              );
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Показываем промежуточны�� результат с МИНИМАЛЬНЫМ порогом для максимальной чувствительности
          if (
            combinedTranscript &&
            combinedTranscript.length > 0 && // МИНИМАЛЬНЫЙ порог - даже 1 символ
            combinedTranscript.length < 150 && // Увеличиваем лимит для длинных команд
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  }, 400); // Умень��аем с 800 до 400мс для быстрого отклика
  ...
  const speak = (text: string) => {
    ...
    console.log("🔊 ��а��инае������ воспроизве��ение:", text);
  ...
  const speakThankYou = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error(
        "Не удал����сь вос��������звести ау��ио благо��а��ности:",
        error,
      );
    });
  };
  ...
  const speakRotateModel = () => {
    // Улучш��нная защ����т�� - раз���ешаем если нет активного ауд��о
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const activateStarkLab = () => {
    ...
    // ПЕРВОЕ аудио для ком��нды "полная ак��ива��ия"
    const firstAudio = new Audio(
  ...
  const processVoiceCommand = (command: string) => {
    ...
    const forceUnlockTimer = setTimeout(() => {
      console.log("�� Принудительное разблокирование через 8 секу����");
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      setIsSpeaking(false);
      lastCommandRef.current = "";
    ...
    if (
      ...
      command.includes("джарв��с верни старую") ||
    ...
    if (
      ...
      // Разрешаем только если нет активного аудио И не в cooldown И последняя команд�� другая
    ...
    if (
      command.includes("про��рутить вверх") ||
  ...
  const toggleListening = () => {
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        // Дополнительная очистка с задержко��
        setTimeout(() => {
          setTranscript("");
          lastCommandRef.current = "";
          commandCooldownRef.current = false;
          audioPlayingRef.current = false;
          console.log("🧹 Дополнительная очистк�� состояний завершена");
        }, 100);
