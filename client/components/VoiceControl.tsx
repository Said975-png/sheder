...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Критическ��я ошибка разрешений - отк��ючае��
  ...
  // Effect для автоматической ��ч��стки застрявши�� блокировок
  useEffect(() => {
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "��� Cooldown сброшен, ми���рофон отключен ил�� ��ома��да отключен��я - не ��ткрывае�� панел��",
        );
      }
    }, delay);
  };
  ...
  const speakAuthenticJarvis = () => {
    // Улучшенная за����и��а - разрешаем если нет ��кт��вного аудио
    if (isSpeaking && audioPlayingRef.current) {
    ...
    console.log("🔊 В��спр��из��о���им аудио, но остав��яем микрофон активным");
  ...
  const activateStarkLab = () => {
    ...
    console.log(
      "�� Активация лаборатории Старка - начинаем последовательнос��ь",
    );
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
  ...
  const processVoiceCommand = (command: string) => {
    ...
    const forceUnlockTimer = setTimeout(() => {
      console.log("⏰ Принудительн��е разблокирование через 8 секунд");
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      setIsSpeaking(false);
      lastCommandRef.current = "";
    ...
    if (
      command.includes("��ригинальный джарв��с") ||
    ...
    // Команды корзины
    if (command.includes("корзина") && command.includes("о�����стить")) {
      clearCart();
    ...
    // Принудительно сбрасываем все б��окировки для нераспознанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
