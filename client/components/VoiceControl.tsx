...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          // З��прашиваем д��ступ к мик��офону с оптимальными настройками
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          // Кри��ические ошибки �� у��ным восстановлени��м
  ...
  const speak = (text: string) => {
    ...
    // Ес��и есть cooldown, но не играет аудио, то ��рин��дительн�� сбрасываем cooldown
  ...
  const speakWithJarvis = async (text: string) => {
    ...
    try {
      ...
      resetState();
      console.error("❌ ��е уд��лось запустить голос Джарвиса:", error);
    }
  };
  ...
  const speakRotateModel = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error(
        "❌ Не удалось воспро��звести аудио для вращения модели:",
        error,
      );
    });
  };
  ...
  const speakCorrect = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ошибка ��ос��роизведени��� втор��го аудио");
    };
  ...
  const changeToOldModel = () => {
    ...
    audio.onended = () => {
      resetState();
      // Отправляем событие возврата к старой ��одели
      window.dispatchEvent(changeEvent);
    };
  ...
  const activateStarkLab = () => {
    ...
    firstAudio.onended = () => {
      ...
      // Воспроизв��дим ВТ��РОЕ аудио после смены дизайна
      setTimeout(() => {
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // ��АРА��ТИРОВ�����ННАЯ защита от зас�����ревания: всегд�� разрешаем обработку новых команд
    ...
    // Очищаем тай����р если команда успешно завершится
    ...
    if (
      ...
      command.includes("джарвис ��оста��очно") ||
    ...
    if (
      ...
      if (
        ...
        found = searchAndNavigate(["��озможно��ти", "мощные", "features"]);
        if (found) {
      ...
      if (
        ...
        command.includes("кт�� мы")
      ) {
        found = searchAndNavigate(["компан", "о нас", "about", "кто мы"]);
        if (found) {
          speak("Показываю информацию о компа��ии");
          return;
        }
      }
