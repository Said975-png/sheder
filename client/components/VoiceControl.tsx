...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        if (
          ...
          // @ts-ignore - Настройки для дальнего распозна��ания
          try {
          ...
          navigator.mediaDevices
            .getUserMedia({
              audio: {
                ...
                volume: 1.0,
                sampleRate: 48000, // В��сокое качество записи
                sampleSize: 16,
                channelCount: 1,
              },
            })
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Обрабатываем только ПОСЛЕДНИЙ резуль��ат, чтобы ���� накапливать старые
          ...
          // Команда отключения имеет абсолютный приоритет и выполняется ��сегда
          ...
          if (
            ...
            // Проверяем, что команда отличается от предыдущей и достаточно длинная (понижен порог)
            if (
              command &&
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  setNoSpeechCount(0); // Сбрасываем счетчик пр�� успешном ра��познавании

                  processVoiceCommand(command);

                  // Быстрая очистка транскрипта посл�� запуска ко��анды
                  setTimeout(() => {
  ...
  const resetCommandState = (
    ...
    setTimeout(() => {
      ...
      if (isListening && !skipPanelReopen) {
        ...
        console.log(
          "✅ Cooldown сброшен, микрофон отключен ил�� команда отключен��я - не ��ткрываем панель",
        );
      }
    }, delay);
  };
  ...
  const speak = (text: string) => {
    ...
    setTranscript("");
    // НЕ вызываем onListeningChange во время воспроизведе��ия ауди��
  ...
  const speakThankYou = () => {
    // Разрешаем выпо��нение если нет активного аудио
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.lang = "en-US"; // ��нг��ийский для лучшего качества, пото�� переклю����им ���� русский
      ...
      // ����сли н�� ��ашли подходящий а����л��йский, ищем русский мужской
  ...
  const speakCorrect = () => {
    // Улучшенная ��ащита - разрешаем если нет акт��вного ��удио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const activateStarkLab = () => {
    ...
    console.log(
      "🔬 А��тивация лаборатории Старка - начинаем последовательность",
    );
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
    ...
    firstAudio.onended = () => {
      ...
      setTimeout(() => {
        ...
        secondAudio.play().catch((error) => {
          ...
          console.error(
            "❌ Не удалось воспрои��вести второе аудио активации:",
            error,
          );
        });
    ...
    firstAudio.onerror = () => {
      ...
      console.error("❌ Ошибка воспроизведения первого а��дио акти��ации");
    };
  ...
  const deactivateStarkLab = () => {
    ...
    audio.play().catch((error) => {
      resetState();
      console.error("❌ Не удалось воспроизвест�� аудио возврата:", error);
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    // При��удительно сбрасываем все блокировки перед обработкой новой команды
    ...
    // Простая очистка транскрипт���� в начале о��работки
    setTranscript("");
    ...
    if (
      ...
      // Принудительно выполняем ко��анду откл��чени�� независимо от сост��яния
      speakShutdown();
      return;
    }
    ...
    if (
      command.includes("джарвис ��ерни мен�� обратно") ||
      ...
      // Улучшенная проверка - разрешаем если нет активно��о аудио
      if (!isSpeaking || !audioPlayingRef.current) {
        deactivateStarkLab();
      }
      return;
    ...
    // Команда "Джарвис давай продо��жим" - воспроизводит первое аудио
    if (
    ...
    if (
      ...
      command.includes("желе��ный ��ело��ек") ||
    ...
    // Команд�� пр��ветствия "Приве�� Джарвис" - улучшенное распознавани�� с защитой от повторов
    if (
      ...
      command.includes("жарвис прив��т") ||
    ...
    if (
      ...
      command.includes("джарв��с ты ��десь") ||
    ...
    // П��ов����я��м, с����ержит л����� команда значимые слова
    ...
    if (
      ...
      // Если ни��его специфичног����� не найдено, поп��обуем ��бщий ��оиск
      if (!found) {
    ...
    if (
      ...
      command.includes("спу��ти���ься к возможностям") ||
