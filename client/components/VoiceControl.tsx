...
export default function VoiceControl({
  ...
  useEffect(() => {
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        // Допо��нительные настройки для Chrome/WebKit - максима��ьная чувств��тельн��сть
        if (
        ...
        try {
          // Запрашиваем д��ст��п к мик��о��ону с оптимальными настр��йками
          navigator.mediaDevices
            ...
            .then((stream) => {
              ...
              if (audioTracks.length > 0) {
                ...
                // Применяем оптимальные настро��ки если ��оддерживаются
                const constraints = {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Пр��ну����ительно сбрасываем застрявшие б��о��ировки если система мол��ит дольше 5 секунд
          const now = Date.now();
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  setTimeout(() => {
                    ...
                    console.log(
                      "��� ��ос��оян��е очищено, Recognition продолжает работать",
                    );
                  }, 2000);
                },
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          console.log("Voice ID не найден, ��спользуем браузерный TTS");
  ...
  const speakSystemDiagnostics = () => {
    ...
    firstAudio.play().catch((error) => {
      resetState();
      console.error(
        "❌ Не удалось ����спроизвести пе��вое ��удио диагностик��:",
        error,
      );
    });
  };
  ...
  const changeToOldModel = () => {
    ...
    audio.play().catch((error) => {
      ...
      // Отправля��м событие даже пр�� ошибке
      window.dispatchEvent(changeEvent);
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (isIncompleteCommand) {
      ...
      setTimeout(() => {
        if (lastCommandRef.current === trimmedCommand) {
          console.log("🔄 ��еполная команда не завершилась, сбрасываем состояние");
          commandCooldownRef.current = false;
          audioPlayingRef.current = false;
          lastCommandRef.current = "";
          setTranscript("");
    ...
    if (
      ...
      command.includes("д��арвис ве��ни старую модель") ||
    ...
    if (command.includes("заказ") || command.includes("оформить заказ")) {
      navigate("/order");
      speak("П��рехожу к оформлению ��аказа");
      return;
    }
    ...
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        setTimeout(() => {
          ...
          commandCooldownRef.current = false;
          audioPlayingRef.current = false;
          console.log("🧹 Дополнительная очистка состояний заве��шена");
        }, 100);
