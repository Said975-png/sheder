...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        recognitionRef.current.onresult = (event) => {
          ...
          // Используем только новый результ��т
            ...
            if (
              ...
              commandDelayRef.current = setTimeout(
                () => {
                  ...
                  // Полная очистка состоян��я ��ома��ды и ��ерезапуск Recognition
                  setTimeout(() => {
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            // Попытка восстановления с ув��личивающейся задержкой
                ...
                setTimeout(() => {
                  ...
                  if (isListening && recognitionRef.current) {
                    try {
                      ...
                      console.log("Перезапу���� ��осле паузы");
                    }
                  }
                }, 2000);
              }
  ...
  const checkSpeechServiceAvailability = async () => {
    try {
      ...
      console.error("❌ Ошибка проверки доступности серви��а:", error);
      return false;
    }
  };
  ...
  const speakIAmHere = () => {
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не удал��сь воспроизв��ст�� ауд��о ����тве��а:", error);
    });
  };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    setTranscript("");
    // НЕ вызываем onListeningChange во время обработки команд��
    ...
    if (
      command.includes("доброе утро д��арвис") ||
    ...
    if (
      ...
      if (
        ...
        console.log(
          "❌ Приветствие ��аблокировано (��ало времени прошло или играет аудио)",
        );
      }
      return;
    }
    ...
    // Ра��шире��ная нави��ация ��о секциям стр��н��ц��
    if (
    ...
    // Если дошли до э��ого места - команда не была распознана
  ...
  const toggleListening = () => {
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        // Принудительно очищаем все состояния пер��д запуском
        setTimeout(() => {
          setTranscript("");
          lastCommandRef.current = "";
        }, 100);
