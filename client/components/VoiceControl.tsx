...
export default function VoiceControl({
    ...
    if ("webkitSpeechRecognition" in window || "SpeechRecognition" in window) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          // @ts-ignore - Увеличиваем уси��ение микрофона
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (isSpeaking && !audioPlayingRef.current && !currentAudioRef.current) {
        console.log(
          "🔄 Принудительны�� ����рос 'говорящего' состояния без аудио",
        );
        setIsSpeaking(false);
        commandCooldownRef.current = false;
      }
  ...
  const speak = (text: string) => {
    ...
    // Это пр��дотвращает повторно�� открыти�� панели после команды отклю��ен��я
    ...
    audio.play().catch((error) => {
      ...
      console.error("Не уд��лось воспроизвести аудио:", error);
    });
  };
  ...
  const speakHowAreYou = () => {
    ...
    if ("speechSynthesis" in window) {
      ...
      utterance.lang = "en-US"; // ��нг��ийский для лучшег�� качества, потом переклю�����им ���� русский
  ...
  const deactivateStarkLab = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ О����бка воспроизведения аудио возврата");
    };
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("��ерни меня обр��тно")
    ) {
    ...
    if (
      ...
      command.includes("давай про��олжим") ||
      command.includes("джарвис продолжим") ||
      command.includes("продолжим джарвис")
    ) {
      console.log("▶️ Команда 'дава�� продолжим' распознана:", command);
    ...
    // Принудительно сбрасываем все блокировк�� для нераспознанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
    ...
    if (isListening) {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          console.log("Распознав��ние уже зап��щено или н��до��ту����но");
        }
      }
    }
  };
