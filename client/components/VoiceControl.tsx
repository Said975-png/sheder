...
export default function VoiceControl({
  ...
  useEffect(() => {
      ...
      if (recognitionRef.current) {
        ...
        try {
          ...
          navigator.mediaDevices
            ...
            .then((stream) => {
              console.log(
                "���� Получен доступ к ��икрофону с улучшенными настройками",
              );
        ...
        recognitionRef.current.onerror = (event) => {
          ...
          if (event.error === "network") {
            ...
            // Поп��тка восстановления с увеличивающейся заде��жкой
    ...
    return () => {
      ...
      // Ос��анавливаем любое воспроизводящееся ����удио при раз��о��тировани��
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (
        ...
        console.log("🧹 Автоматическая очист���� застрявших блокировок");
        commandCooldownRef.current = false;
        audioPlayingRef.current = false;
        lastCommandRef.current = "";
        setTranscript("");
  ...
  useEffect(() => {
    ...
    const activityTracker = setInterval(() => {
      ...
      // Обновляем время последней активности при изме��ениях
  ...
  const forceResetAllStates = () => {
    console.log("🔥 ПРИН��Д��Т��Л����НЫЙ СБРОС ВСЕХ СОСТОЯНИЙ");
  ...
  const speak = (text: string) => {
    ...
    // ��сли есть cooldown, но не играет аудио, то ��рин��дительн�� сбрасыв��ем cooldown
  ...
  const speakAuthenticJarvis = () => {
    // Улучшенная за�����и��а - ��азрешаем е����и нет акт��вного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const testAudioUrls = () => {
    ...
    fetch(url2)
      .then((response) => console.log("✅ URL2 ��оступен:", response.status))
  ...
  const speakContinue = () => {
    ...
    audio.onerror = () => {
      resetState();
      console.error("❌ Ошибка воспроизведе��ия перв��г�� аудио");
    };
  ...
  const changeToNewModel = () => {
    ...
    console.log("🔄 См��на на новую мо��ель с эффектами");
    setIsSpeaking(true);
    commandCooldownRef.current = true;
    audioPlayingRef.current = true;
  ...
  const speakLoveYou = () => {
    // Улучшен��ая з��щита - разрешаем если нет активного аудио
    if (isSpeaking && audioPlayingRef.current) {
  ...
  const deactivateStarkLab = () => {
    ...
    // ТРЕТЬ�� аудио для коман��ы "ве����и меня обр��тно"
    const audio = new Audio(
  ...
  const processVoiceCommand = (command: string) => {
    ...
    if (
      ...
      command.includes("джарв��с смени мод��ль") ||
    ...
    // Прин��дитель��о сбрасываем все блокировк�� для нераспознанных команд
    setTimeout(() => {
      commandCooldownRef.current = false;
      audioPlayingRef.current = false;
      lastCommandRef.current = "";
