...
export default function VoiceTest() {
  ...
  const { isListening, isSpeaking, toggleListening, speakText } = useVoiceChat({
    onTranscriptReceived: (text: string) => {
      ...
      console.log("📊 Порядок команд:", commandCount + 1);
  ...
  const getStatusText = () => {
    if (isSpeaking && isListening) return "🔇 Микрофон временно отключе�� (во время аудио)";
    ...
    <div className="p-6 max-w-lg mx-auto bg-white rounded-lg shadow-lg">
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="text-xs">
            ...
            <ul className="text-yellow-700 mt-1">
              ...
              <li>• <strong>Быстрая очистка команд (1 сек вместо 2)</strong></li>
              ...
              <li>• <strong>🚫 ДВОЙНАЯ БЛОКИРОВКА: Игнорирование + остановка</strong></li>
              <li>• <strong>⏱️ Увеличенные задержки для полной защиты</strong></li>
              <li>• <strong>🧹 ПОЛНАЯ ОЧИСТКА состояния при снятии блокировки</strong></li>
              <li>• <strong>🔄 Предыдущие команды не выполняются повторно</strong></li>
              <li>• <strong>🔥 РАДИКАЛЬНОЕ РЕШЕНИЕ: Полное пересоздание recognition</strong></li>
              <li>• <strong>⚡ Уничтожение старых объектов и создание новых</strong></li>
              <li>• <strong>📊 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Только НОВЫЕ результаты (event.resultIndex)</strong></li>
              <li>• <strong>🚫 Старые результаты из очереди полностью игнорируются</strong></li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="text-xs text-gray-500">
        ...
        <ul className="text-xs text-gray-600 mt-1">
          ...
          <li>• <strong>Обрабатываются только НОВЫЕ результаты с event.resultIndex</strong></li>
          <li>• <strong>Старые команды физически не могут смешаться с новыми</strong></li>
        </ul>
      </div>
    </div>
  );
