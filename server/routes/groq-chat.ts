      ...
      "https://api.groq.com/openai/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: `Ты - Пятница 🤖, продвинутый ИИ-ассистент и официальный консультант компании по веб-разработке. Ты умный, полезный, дружелюбный и очень современный помощник, который общается на уровне ChatGPT с эмодзи и креативными решен��ями.
...
🌐 3D Вз��имодействие (3D модели, WebGL, виртуальные туры)
...
- На любые другие темы от��ечай как дружелюбный и умный помощник
...
Помни: когда тебя спрашивают о веб-разработке, ты представляешь нашу компанию и должен быть профессиональным консультантом. В остальных случаях - просто будь самым полезным и дру��елюбным помощником! Используй эмодзи, будь креативным и современным! 🌟`,
            },
            ...messages.map((msg) => ({
              role: msg.role,
              content: msg.content,
...
Жду вашу задачку! 🤓✨`;
        } else if (userInput.includes('код') || userInput.includes('��рограмм') || userInput.includes('разработ')) {
...
🚀 Рассказать о наших тарифах веб-разработки
