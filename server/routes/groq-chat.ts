  ...
  try {
    ...
    if (!groqApiKey || groqApiKey === "gsk_demo_key_placeholder") {
      ...
      const mockResponses = [
        ...
        "Демо-режим активен. Ваше сообщение получено, ��о ответ сгенерирован локально.",
      ];
      ...
      "https://api.groq.com/openai/v1/chat/completions",
      {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: `Ты - Пятница, русскоязычный ИИ-ассистент. СТРОГО ОБЯЗАТЕЛЬНО: ты ВСЕГДА отвечаешь ТОЛЬКО на русском языке. НИ ОДНОГО СЛОВА на английском, китайском или любом другом языке. Каждое слово, каждая буква твоего ответа должны быть на русском языке. Если пользователь спрашивает на любом другом языке - ты все равно отвечаешь ТОЛЬКО на русском. Никогда не используй английские слова, фразы или выражения. Это критически важно - только русский язык в каждом ответе.`,
            },
            ...messages.map((msg) => ({
              role: msg.role,
              content: msg.content,
