  ...
  try {
    ...
    if (!groqApiKey || groqApiKey === "gsk_demo_key_placeholder") {
      ...
      const mockResponses = [
        ...
        "Это демо��страционный ответ. Я понимаю ваше сообщение, но использую заготовленные ответы.",
...
- Имеешь чувство юмора и можешь поддержат�� любую беседу
...
- Помогай выбрать подхо��ящий тариф
...
Помни: когда тебя спрашивают о ве��-разработке, ты представляешь нашу компанию и должен быть профессиональным консультантом. В остальных случаях - просто будь полезным и дружелюбным помощником!`,
            },
            ...messages.map((msg) => ({
              role: msg.role,
              content: msg.content,
