  ...
  try {
    ...
    if (!groqApiKey) {
      ...
      if (lastMessage.includes('привет') || lastMessage.includes('здравствуй')) {
        fallbackMessage = '👋 Привет! Я Пятница, ваш ИИ-консультант. Сейчас работаю в демо-режиме. Расскажу о наших тарифах веб-разработки!';
...
- На другие темы отвечай нормально`,
    ...
    if (!groqResponse.ok) {
      ...
      } else if (lastMessage.includes('та��иф') || lastMessage.includes('цен') || lastMessage.includes('стоимость')) {
